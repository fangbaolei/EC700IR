...
{
    return g_pTrafficGate->ForceSend(dwVideoID);
}

HRESULT GetWorkModeInfo(SYS_INFO* pInfo)
{
    return(g_pTrafficGate->GetWorkModeInfo(pInfo));
}

HRESULT GetWorkModeList(SYS_INFO* pInfo)
{
    return(g_pTrafficGate->GetWorkModeList(pInfo));
}
...
HRESULT CTrafficGate::ForceSend(DWORD32 dwVideoID)
{
...
HRESULT CTrafficGate::GetWorkModeInfo(SYS_INFO* pInfo)
{
...
HRESULT CTrafficGate::GetWorkModeList(SYS_INFO* pInfo)
{
...
void CTrafficGate::OnImage(void* pContext, IMG_FRAME imgFrame)
{
    CTrafficGate * pThis = (CTrafficGate *)pContext;
...
HRESULT CTrafficGate::Run(void* pvParam)
{
...
HRESULT CTrafficGate::SetAWBFromLightType(LIGHT_TYPE emLightType)
{
...
HRESULT CTrafficGate::ScaleCarFlowRate()
{
...
void CTrafficGate::DspSoftTrigger()
{
#ifdef SINGLE_BOARD_PLATFORM
    if (!m_pCamTrigger)
    {
...
HRESULT CTrafficGate::CarArrive(
...
bool CTrafficGate::ThreadIsOk(int* piErrCode)
{
...
void CTrafficGate::SetModuleParams(const ModuleParams& cParam)
{
    m_cModuleParams = cParam;
}

void CTrafficGate::GetCarColor(CARLEFT_INFO_STRUCT *pCarLeftInfo, char *pszConf)
{
...
void CTrafficGate::GetCarType(PROCESS_IMAGE_CORE_RESULT *pImgCoreInfo, char *pszConf)
{
...
void CTrafficGate::GetEventDetInfo(CARLEFT_INFO_STRUCT *pCarLeftInfo, char *pszConf)
{
    pszConf[0] = 0;
    ...
    //超速
    if ((int)(pCarLeftInfo->cCoreResult.fltCarspeed + 0.5f) >
...
HRESULT CTrafficGate::SetCaptureSynSignalEnableFlash(bool fIsNight)
{
    HRESULT hr = E_FAIL;
