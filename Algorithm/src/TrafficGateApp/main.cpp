...
CTrafficGate *g_pTrafficGate = NULL;
...
int main(int argc, char** argv)
{
    ...
    // 创建卡口数据流控制对象
    g_pTrafficGate = new CTrafficGate();
    g_pTrafficGate->SetModuleParams(cModuleParams);
    g_pTrafficGate->SetFlashLampDiff(cModuleParams.cSignalMatchParam.iFlashLampDiff);

    // 挂载功能组件
    g_pTrafficGate->MountVideoRecoger(pVideoRecoger);
    g_pTrafficGate->MountVideoGetter(pVideoGetter);
    g_pTrafficGate->MountResultSender(pResultSender);
    g_pTrafficGate->MountSignalMatch(pOuterControl);
    g_pTrafficGate->MountImgCapturer(pImgCapturer);
    g_pTrafficGate->MountTriggerCam(pCamTrigger);
    g_pTrafficGate->MountLightTypeSaver(&cCamLightTypeSaver);
#ifdef SINGLE_BOARD_PLATFORM
    g_pTrafficGate->MountVideoSender(&videoLinkCtrl);
#else
    ...
    // 注意：总控线程一定要最后再启动
    if (S_OK != g_pTrafficGate->Start(NULL))
    {
...
#else
        dwCurTick = GetSystemTick();
        if (dwCurTick - g_pTrafficGate->GetLastResultTime() >= CPU_THROB_PERIOD)
        {
            ...
            {
                //更新时标
                g_pTrafficGate->SetLastResultTime(dwCurTick);
            }
            else
            {
...
#endif
        int iErrCode=0;
        if ( !g_pTrafficGate->ThreadIsOk(&iErrCode) )
        {
            if (1 == iErrCode)
            {
