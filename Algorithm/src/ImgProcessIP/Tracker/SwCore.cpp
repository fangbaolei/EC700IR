...
void SetFuctionCallBack(PLATFORM_FUCTION_LIST* pcList)
{
	if( g_pFuction == NULL )
	{
		...
		g_pFuction->pfOutPutDebugInfo = pcList->pfOutPutDebugInfo;
	}
}
...
// 动态更新车辆检测模型
int UpdateModel(void* pvParam, int iSize, void* pvRespond, int iRespondSize);
// 动态更新算法参数
int UpdateAlgParam(void* pvParam, int iSize, void* pvRespond, int iRespondSize);
...
//	int iInputSize = input->len;
	...
	if( DCI_INIT_VIDEO_RECOGER != nCmdId
			...
			&& DCI_UPDATE_MODEL_DATA != nCmdId
			&& DCI_UPDATE_ALG_PARAM != nCmdId )
	{
		return -3;
	}
	...
	else if( DCI_UPDATE_MODEL_DATA == nCmdId )
	{
		return UpdateModel(pvParam, iSize, pvRespond, iRespondSize);
	}
	else if( DCI_UPDATE_ALG_PARAM == nCmdId )
	{
		return UpdateAlgParam(pvParam, iSize, pvRespond, iRespondSize);
	}
...
int UpdateAlgParam(void* pvParam, int iSize, void* pvRespond, int iRespondSize)
{
	...
	if( NULL != pfDebugOutPut )
	{
		sprintf(szMsg, "<UpdateAlgParam>input:%08x,%08x, %d. output:%08x,%08x, %d.\n",
				input->addr, input->phys, input->len, output->addr, output->phys, output->len);
		pfDebugOutPut(szMsg);
	}
	...
	// 只处理指定命令
	if( DCI_UPDATE_ALG_PARAM != nCmdId )
	{
		return -3;
	}
	...
	if( NULL != pfDebugOutPut )
	{
		sprintf(szMsg, "<UpdateAlgParam>param size:%d.\n",
				*pnParamSize);
		pfDebugOutPut(szMsg);
	}
	...
	int iRet = 0;
	// 判断参数长度是否正确
	if (*pnParamSize <= 1024 * 1024)
	{
		pbPos += 4;
		unsigned char* pbParam = (unsigned char*)pbPos;
		if( g_pTrackerVideo == NULL )
		{
			pRespondHead->iResult = S_OK;
		}
		...
		{
			pRespondHead->iResult = g_pTrackerVideo->UpdateAlgParam(pbParam, (*pnParamSize));
		}


		if( NULL != pfDebugOutPut )
		{
			sprintf(szMsg, "<UpdateAlgParam>UpdateAlgParam ret:%08x.\n",
					pRespondHead->iResult);
			pfDebugOutPut(szMsg);
		}
	...
	else
	{
		...
		sprintf(
			pRespondHead->szErrorInfo,
			"UpdateAlgParam failed, the size of param error!!!\n"
			);
		iRet = -5;
	}
...
int UpdateModel(void* pvParam, int iSize, void* pvRespond, int iRespondSize)
{
	...
	if( NULL != pfDebugOutPut )
	{
		sprintf(szMsg, "<UpdateModel>input:%08x,%08x, %d. output:%08x,%08x, %d.\n",
				input->addr, input->phys, input->len, output->addr, output->phys, output->len);
		pfDebugOutPut(szMsg);
	}
	...
	// 只处理指定命令
	if( DCI_UPDATE_MODEL_DATA != nCmdId )
	{
		return -3;
	}
	...
	int* pnParamSize = (int*)pbPos;
	pbPos += 4;
	unsigned int* pnModelId = (unsigned int*)pbPos;
	...
	if( NULL != pfDebugOutPut )
	{
		sprintf(szMsg, "<UpdateModel>param size:%d, %d.\n",
				*pnParamSize, *pnDataSize);
		pfDebugOutPut(szMsg);
	}
	...
	if (*pnParamSize == 4)
	{
		...
		{
			pRespondHead->iResult = g_pTrackerVideo->UpdateModel((*pnModelId), pbParam, (*pnDataSize));
		}

		if( NULL != pfDebugOutPut )
		{
			sprintf(szMsg, "<UpdateModel>UpdateModel ret:%08x.\n",
					pRespondHead->iResult);
			pfDebugOutPut(szMsg);
		}
	...
	else
	{
		...
		sprintf(
			pRespondHead->szErrorInfo,
			"UpdateModel failed, the size of param error!!!\n"
			);
		iRet = -5;
	}
