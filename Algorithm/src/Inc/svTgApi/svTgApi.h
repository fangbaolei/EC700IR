...
namespace svTgApi
{
    ...
    struct TG_TRACK_EX_INFO
    {
        ...
        int nObjAvgGray;                /// 物体平均亮度，仅当为非机动车且使能 nEnableCalcObjAvgGray 开关时才有效
    };
    ...
    struct TG_PARAM
    {
        ...
        // 车道标识座标，注意只能从左起，依次往右
        SV_ROAD_INFO rgRoadLine[3]; /// 车道线, 目前最大只支持2车道
        ...
        sv::SV_BOOL nEnableCalcObjAvgGray;               /// 使能计算物体平均灰度

        /// 触发抓拍线
        int nTriggerLine;  

        /// 图像固定延时时间，用于计算实际抓拍触发位置
        int nImageConstDelay;
        ...
        TG_PARAM()
            ...
            ,   nPlateOutLineMode(0)
            ,   nEnableCalcObjAvgGray(FALSE)
            ,   fEnableCalcSpeed(FALSE)
            ,   fltRoadWidth(3.75f)
            ,   fltRoadLength(40.0f)       
            ,   nTriggerLine(60)
            ,   nImageConstDelay(100)
        {
    ...
    public:
        ...
        /// 获取跟踪实时速度
        virtual sv::SV_FLOAT GetTrackerCurSpeed(int nID) = 0;    
