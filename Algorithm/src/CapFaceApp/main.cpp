...
/* DataCtrl.cpp */
extern void DspSoftTriggerCallback(int unused);

void PrintfErrorInfo(SLAVE_CPU_STATUS *pSlaveCapStatus, char *pszErrInfo);

HRESULT InitMemBlocksEx(int iCamType);
...
int main(int argc, char** argv)
{
...
#ifdef SINGLE_BOARD_PLATFORM
    //申请保存抓拍信息的共享内存
    CreateShareMemOnPool(&cModuleParams.cTrackerCfgParam.cTriggerInfoBuf, sizeof(TRIGGER_CAM_INFO));
#endif
    // 初始化DSPLink
    ...
    if (g_cHvDspLinkApi.SetSoftTriggerCallback(DspSoftTriggerCallback) != S_OK)
    {
        HV_Trace(5, "SetSoftTriggerCallback failed!\n");
        return -1;
    }
    ...
    cModuleParams.cSignalMatchParam.fEnablePanoramicCamera =
        (cModuleParams.cResultSenderParam.iEnablePanoramicCamera == 1) ? true : false;
    ...
    case 4: //HV摄像机协议
    {
        ...
        ((CVideoGetter_HvCamera*)pVideoGetter)->MountSignalMatch(pOuterControl);
        ((CVideoGetter_HvCamera*)pVideoGetter)->SetCapCamCfgParam(&(cModuleParams.cResultSenderParam.cCapCamParam));
        HV_Trace(5, "HV摄像机协议\n");
        break;
    }
    case 2://创宇协议
    ...
    cPciParam.cPanoramicCamaraParam.nEnableFlag
        = cModuleParams.cResultSenderParam.iEnablePanoramicCamera;
    memcpy(cPciParam.cPanoramicCamaraParam.szCamaraIP,
        cModuleParams.cResultSenderParam.szPanoramicCameraIP, 20);
    // 启动PCILink
    ...
    // 创建卡口数据流控制对象
    g_pCapFace = new CCapFace();
    g_pCapFace->SetModuleParams(cModuleParams);
    g_pCapFace->SetFlashLampDiff(cModuleParams.cSignalMatchParam.iFlashLampDiff);

    // 挂载功能组件
    g_pCapFace->MountVideoRecoger(pVideoRecoger);
    g_pCapFace->MountVideoGetter(pVideoGetter);
    g_pCapFace->MountResultSender(pResultSender);
    g_pCapFace->MountEventChecker(pEventChecker);
    g_pCapFace->MountSignalMatch(pOuterControl);
    g_pCapFace->MountImgCapturer(pImgCapturer);
    g_pCapFace->MountTriggerCam(pCamTrigger);
    g_pCapFace->MountLightTypeSaver(&cCamLightTypeSaver);
#ifdef SINGLE_BOARD_PLATFORM
    g_pCapFace->MountVideoSender(&videoLinkCtrl);
#else
...
#else
        dwCurTick = GetSystemTick();
        if (dwCurTick - g_pCapFace->GetLastResultTime() >= CPU_THROB_PERIOD)
        {
            ...
            {
                //更新时标
                g_pCapFace->SetLastResultTime(dwCurTick);
            }
            else
            {
...
#endif
        int iErrCode=0;
        if ( !g_pCapFace->ThreadIsOk(&iErrCode) )
        {
            if (1 == iErrCode)
            {
