...
public:
  ...
  //错误时返回-1, 否则返回实际读取数目,如果没有更多数据设置eof_flag为真
...
private:
  ...
  void H2V2Convert( bool fYUV = FALSE);
  void H2V1Convert( bool fYUV = FALSE);
  void H1V2Convert( bool fYUV = FALSE);
  void H1V1Convert( bool fYUV = FALSE);
  void GrayConvert( bool fYUV = FALSE);
...
public:	
  ...
  //取得下一个扫描行及长度,返回JPGD_DONE表示解码结束
  int decode(void * *Pscan_line_ofs, uint *Pscan_line_len, bool fYUV = false );
