...
*  CTollGete控制类
*/
class CTollGate : public CHvThreadBase, public IDataCtrl
{
public:
    CTollGate();
    ~CTollGate();
    ...
    void GetNoPlateInfo(CARLEFT_INFO_STRUCT& stCarLeftInfo, IReferenceComponentImage* pRefImage);
    bool IsMatchPlateFT(DWORD32 dwTimeNow, DWORD32 dwLastPlateTime,
                      DWORD32 dwPrevTime, DWORD32 dwPostTime);
    void OutPutNoPlateFT();
...
private:
    ...
    int m_iOutPutNoPlate;
    int m_iPrevTime;
    int m_iPostTime;
    bool m_fIsMatch;
    IReferenceComponentImage* m_pRefImage;

    CQueue<IMG_FRAME, IMG_LIST_COUNT> m_queImage;
};

extern CTollGate *g_pTollGate;
