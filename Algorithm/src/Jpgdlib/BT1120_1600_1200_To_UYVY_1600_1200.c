...
)
{
	...
	for ( i=2; i<IMAGE_HEIGHT; i+=1, ++k )
	{
		...
		//CACHE_invL2(pbCbYCrY + LINE_SIZE_YUV422*2*(k-1), LINE_SIZE_YUV422*2, CACHE_WAIT);
		ProcessDoubleLine(
			(unsigned char *)g_bBufLine1, (unsigned char *)g_bBufLine2, 
			(unsigned char *)pbCbYCrY + LINE_SIZE_YUV422*2*(k)
			);
	}
	HV_dmawait_dm6467(h1_next);
	HV_dmawait_dm6467(h2_next);
	...
	HV_dmawait_dm6467(h1_output);
	//CACHE_invL2(pbCbYCrY + LINE_SIZE_YUV422*2*(k), LINE_SIZE_YUV422*2, CACHE_WAIT);
}
