		...
		)
{
	...
	{
		nBottomLineX0 = GetLineX(nBottomLine, 0);
		nBottomLineX1 = GetLineX(nBottomLine, CTrackInfo::m_iRoadNumber - 1);
		nTopLineX0 = GetLineX(nTopLine, 0);
		nTopLineX1 = GetLineX(nTopLine, CTrackInfo::m_iRoadNumber - 1);
	}
	else
	{
		nBottomLineX0 = GetLineX(nBottomLine, CTrackInfo::m_iRoadNumber - 1);
		nBottomLineX1 = GetLineX(nBottomLine, 0);
		nTopLineX0 = GetLineX(nTopLine, CTrackInfo::m_iRoadNumber - 1);
		nTopLineX1 = GetLineX(nTopLine, 0);
	}
	...
	m_rcVideoDetArea.left = 0;//m_cDetAreaInfo.DetectorAreaLeft;
	m_rcVideoDetArea.top = 0;//m_cDetAreaInfo.DetectorAreaTop;
	m_rcVideoDetArea.right = 100;//m_cDetAreaInfo.DetectorAreaRight;
	...
	)
{
	...
	const int MAX_FACE_POS_COUNT = 20;
	RECTA rgFacePos[MAX_FACE_POS_COUNT];
	CRect rcCrop;
	...
	CRect rcFaceCrop;
	rcFaceCrop.left = (long)(imgFrame.iWidth * (rcRecogArae.left + rcRecogArae.Width()/3)/100.0);
	rcFaceCrop.top = (long)(imgFrame.iHeight * rcRecogArae.top/100.0);
	rcFaceCrop.right = (long)(imgFrame.iWidth * (rcRecogArae.right - rcRecogArae.Width()/3)/100.0);
	rcFaceCrop.bottom = (long)(imgFrame.iHeight * (rcRecogArae.bottom - rcRecogArae.Height()/4)/100.0);
	HV_COMPONENT_IMAGE imgFaceCrop;
	CropImage(imgFrame, rcFaceCrop, &imgFaceCrop);
	...
	// 如果检测不到车牌则进行人脸检测
	if (MultiROI.dwNumROI == 0 && m_fEnableDetFace)
	{
		if( S_OK == m_pPlateDetector->ScanPlatesComponentImage(
			FACE_DETECTOR,
			&imgFaceCrop, 
		...
		{
			PROCESS_IMAGE_CORE_RESULT* pCurResultTemp;
			// 将人脸信息存放在第一个CarLeft结构体中
			pCurResultTemp = &(pProcessRespond->cTrigEvent.rgCarLeftCoreInfo[0]);
			pCurResultTemp->nFaceCount = (int)MultiROI_Face.dwNumROI;
			//调整检出位置为绝对坐标
			for(int j = 0; j < MultiROI_Face.dwNumROI; j++)
			{
				RECTA* pROI = MultiROI_Face.rcROI + j;
				pROI->left += rcFaceCrop.left;
				pROI->top += rcFaceCrop.top;
				pROI->right += rcFaceCrop.left;
				pROI->bottom += rcFaceCrop.top;
				pCurResultTemp->rgFaceRect[j] = (*pROI);
			}
		}
	}
	
	pProcessRespond->iFrameAvgY = m_iAvgY;
	int nInfoCount = 0;
	PlateInfo rgPlateInfo[MAX_EVENT_COUNT];
	CRect rcMax(0,0,imgCrop.iWidth,imgCrop.iHeight);
	for(int i = 0; i < MIN_INT(MultiROI.dwNumROI,MAX_EVENT_COUNT); i++)
	{
		RECTA *pROI = MultiROI.rcROI + i;
		CRect rcCurPlate(*pROI);

		int nWidth = rcCurPlate.Width();
		rcCurPlate.left -= (nWidth * 140) >> 10;
		rcCurPlate.right += (nWidth * 140) >> 10;

		if (!rcCurPlate.IntersectRect(&rcCurPlate, &rcMax)) continue;

		PlateInfo cInfo;
		cInfo.rcPos.top = rcCurPlate.top + rcCrop.top;   //车牌在全图中的位置
		cInfo.rcPos.left = rcCurPlate.left + rcCrop.left;
		cInfo.rcPos.bottom = rcCurPlate.bottom + rcCrop.top;
		cInfo.rcPos.right = rcCurPlate.right + rcCrop.left;

		cInfo.color = pROI->color;
		cInfo.nPlateType = pROI->nPlateType;
		cInfo.iLastDetWidth = pROI->right-pROI->left;
		cInfo.nVariance = pROI->nVariance;

		HV_COMPONENT_IMAGE imgPlate;
		CropImage(imgCrop, rcCurPlate, &imgPlate);
		...
		imgRectifyPlate.Assign(imgPlate);

		if(S_OK != RecognizePlate(imgPlate, cInfo, imgRectifyPlate))
		{
			continue;
		}

		rgPlateInfo[nInfoCount++] = cInfo;
		
		switch(cInfo.nPlateType)
		{
		case PLATE_NORMAL:				// 蓝牌或黑牌
			if ((cInfo.rgbContent[0] == 57) && (cInfo.rgbContent[1] == 36))
			{
				// 前两位为“粤Z”
				if ((cInfo.rgbContent[6] != 114) && (cInfo.rgbContent[6] != 115))		// 港澳字尾
				{
					cInfo.rgbContent[6] = 115;
				}
				cInfo.color = PC_BLACK;			// 强制设为黑色
			}
			break;
		case PLATE_POLICE:				// 黄牌、军牌或广西警牌
			if (IsMilitaryAlpha(cInfo.rgbContent[0]) || IsMilitaryChn(cInfo.rgbContent[0]) || ((cInfo.rgbContent[0] == 38) && (cInfo.rgbContent[1] == 32))) //加入新军牌首字符为字母的判断
			{
				cInfo.color = PC_WHITE;			// 强制设为白色
				break;
			}

			// (字头中已经没有军牌了，只能是黄牌和广西警牌)
			if (cInfo.rgbContent[6] == 37)		// 最后一位为“警”
			{
				if (cInfo.rgbContent[0] != 58)	// 但第一位非“桂”
				{
					// 标准黄牌或浅蓝牌
					if (cInfo.color == PC_WHITE)
					{
						cInfo.color = PC_YELLOW;			// 强制设为黄色,也可能是浅蓝牌
					}
				}
				else
				{
					// 广西警牌
					cInfo.color = PC_WHITE;			// 强制设为白色
					cInfo.nPlateType = PLATE_POLICE2;
				}
			}
			else
			{
		...
		RecognizeCarColour(imgFrame, cInfo,m_iAvgY);   //车身颜色传进去的图片应该是全图而不是截图
		...
		if (m_fEnableDetFace)
		{
			HV_COMPONENT_IMAGE imgFace;
			CRect rcFace = rcCurPlate;
			rcFace.InflateRect(rcFace.Width() * 2, rcFace.Width() * 6);
			if( rcFace.left < 0 ) rcFace.left = 0;
			if( rcFace.top < 0 ) rcFace.top = 0;
			if( rcFace.right > rcCrop.right ) rcFace.right = rcCrop.right;
			if( rcFace.bottom > rcCrop.bottom ) rcFace.bottom = rcCrop.bottom;
			if( rcFace.Height() > (rcCrop.Height() * 2 / 3)) rcFace.top += rcFace.Height() / 5;

			CropImage(imgFrame, rcFace, &imgFace);
			if( S_OK == m_pPlateDetector->ScanPlatesComponentImage(
				FACE_DETECTOR,
				&imgFace, 
				&MultiROI_Face, 
			...
			{
				pCurResult->nFaceCount = (int)MultiROI_Face.dwNumROI;
				//调整检出位置为绝对坐标
				for(int j = 0; j < MultiROI_Face.dwNumROI; j++)
				{
					RECTA* pROI = MultiROI_Face.rcROI + j;
					pROI->left += rcFace.left;
					pROI->top += rcFace.top;
					pROI->right += rcFace.left;
					pROI->bottom += rcFace.top;
					pCurResult->rgFaceRect[j] = (*pROI);
				}
			}
		}

		HV_COMPONENT_IMAGE imgBestPlate = imgPlate;
		CPersistentComponentImage imgTemp;
		if(SUCCEEDED(imgTemp.Create(HV_IMAGE_YUV_422, imgBestPlate.iWidth, 2*imgBestPlate.iHeight)))
		{
			PlateResize(imgBestPlate, imgTemp);
			imgBestPlate = imgTemp;
		}
		...
		pCurResult->rcBestPlatePos = rcCurPlate;
		...
		pCurResult->fIsNight = m_LightType == NIGHT;
		pCurResult->nPlateLightType = m_PlateLightType;
		pCurResult->iCplStatus = m_iCplStatus;
		pCurResult->iPulseLevel = m_iPulseLevel;
		pCurResult->nRoadNo = MatchRoad(rcCurPlate.CenterPoint().x, rcCurPlate.CenterPoint().y);
		pCurResult->iObservedFrames = 1;
		pProcessRespond->cTrigEvent.dwEventId |= EVENT_CARLEFT;
		pProcessRespond->cTrigEvent.iCarLeftCount++;
	}
	HV_Trace(1, "\nRecogSnap Done! %d\n", GetSystemTick() - dwBegin);

	return S_OK;
}


DWORD32 dwTotalFrameTime(0), dwFrameNum(0);
...
{
	m_nIOLevel = pParam->dwIOLevel;

	if(pParam->iCarArriveTriggerType == 1)
	{
		m_fEnableCarArriveTrigger = 0;
	}
	else if(pParam->iCarArriveTriggerType == 2)
	{
		m_fEnableCarArriveTrigger = 1;
	}
	g_szDebugInfo[0] = 0;
	//g_iDiffFPGATick = pParam->iDiffTick;
	// zhaopy
	//pProcessRespond->cFrameStateInfo.nCount = 0;
	//pProcessRespond->cFrameStateInfo.rgFrameState[pProcessRespond->cFrameStateInfo.nCount].dwTimeTick = pParam->dwImageTime;
	//pProcessRespond->cFrameStateInfo.rgFrameState[pProcessRespond->cFrameStateInfo.nCount++].nState = FS_NORMAL;
	//m_nIOLevel = pParam->dwIOLevel;
	
	HV_COMPONENT_IMAGE& imgSnapFrame = pProcessData->hvImageYuv;
    if ((imgSnapFrame.iWidth != m_nWidth || imgSnapFrame.iHeight != m_nHeight) || m_fIsModifyParam)
	{
    	// 规避EC700偶发性BUG
    	if (imgSnapFrame.iWidth >= 3300 && imgSnapFrame.iHeight <= 500)
    	{
    		sv::utTrace("[xxx]Error height = %d\n", imgSnapFrame.iHeight);
    		return S_OK;
    	}
        ...
        RTN_HR_IF_FAILED(m_cTgTrackCtrl.Init(&m_cTrackerCfgParam, m_pParam, m_nWidth, m_nHeight));

		m_fIsModifyParam = false;			

	}

    pProcessRespond->cTrigEvent.iCarLeftCount = 0;

	//sprintf(g_szDebugInfo + strlen(g_szDebugInfo), "Pre Pro\n");
	//Venus_OutputDebug(g_szDebugInfo);

	static int s_iSumTime = 0; 
	static int s_dwFrameCnt = 0; 
	DWORD32 dwStartTime = GetSystemTick(); 

    //dbg
    sv::SV_UINT32 nCurTime = sv::utGetSystemTick();
    static sv::SV_UINT32 s_nLastOutTime = 0;

    static int s_nDelayCount = 0;
    static int s_MaxTime = 0;
    static int s_nDelayAll = 0;
    static int s_nDelayAllCount = 0;
    int _nUseTime = nCurTime - s_nLastOutTime;
    if (s_nLastOutTime != 0 && _nUseTime > s_MaxTime)
    {
        s_MaxTime = _nUseTime;
    }
    if (++s_nDelayCount > 20)
    ...
    if (s_MaxTime > 20)
    {
        sv::utTrace("@CtrlImpl frame in max delay:%d\n", s_MaxTime);
    }
    s_nDelayAll += _nUseTime;
    ++s_nDelayAllCount;
    //dbg

    // 算法处理调用
    HRESULT hr = m_cTgTrackCtrl.Process(pParam, pProcessData, pProcessRespond);

    //dbg 记录执行完的时间
    s_nLastOutTime = sv::utGetSystemTick();

    if (s_nDelayAllCount > 60)
    {
    	sv::utTrace("@CtrlImpl Delay Avg %d\n", s_nDelayAll/s_nDelayAllCount);
    	s_nDelayAllCount = 0;
    	s_nDelayAll = 0;
    }

	int iUseTime = (int)(GetSystemTick() - dwStartTime);

   	s_iSumTime += iUseTime;
   	++s_dwFrameCnt;

	/*char szAvrTime[64];
	int iInfoLen = strlen(g_szDebugInfo);
	sprintf(szAvrTime, "@T%d, A%.2f\n" , iUseTime, (float)s_iSumTime/s_dwFrameCnt);
	if (3095 > (iInfoLen +  strlen(szAvrTime)))
	{
		strcat(g_szDebugInfo, szAvrTime);
	}*/
	if (s_dwFrameCnt > 10000)
	{
		s_dwFrameCnt = 0;
		s_iSumTime = 0;
	}
...
)
{
	...
	{
		//车牌无牌车检测结果
		svTLDetApi::SV_PLATERECT* rgPlateRect = NULL;
		int nAvgY = 100;
		int nPlateCount = 0;
	    svTLDetApi::DET_ROI*  rgRoi = NULL;
	    int nRoiCount = 0;
	
		if (pProcessData->cSyncDetData.pbData != NULL)
		{
			sv::SV_RESULT hr = svTLDetApi::SeparateResultRefPoint(pProcessData->cSyncDetData.pbData,
					pProcessData->cSyncDetData.nLen,
					&nAvgY, &rgPlateRect, &nPlateCount, &rgRoi, &nRoiCount);

			if (rgPlateRect == NULL)   nPlateCount = 0;
			if (hr == sv::RS_S_OK)
			{
	            if (nAvgY != -1)  m_iAvgY = nAvgY;
			}
			else
			{
				Venus_OutputDebug("sync data extract err!\n");
			}

	        m_cDetected = nRoiCount;
	        for (int i=0; i<nRoiCount; ++i)
	        {
	            /*sprintf( szMsg, "Sync ROI [%d %d %d %d]\n",
	                rgRoi[i].m_nLeft,
	                rgRoi[i].m_nTop,
	                rgRoi[i].m_nRight,
	                rgRoi[i].m_nBottom
	                );*/
	            m_rgrcDetected[i].left = rgRoi[i].m_nLeft;
	            m_rgrcDetected[i].top = rgRoi[i].m_nTop;
	            m_rgrcDetected[i].right = rgRoi[i].m_nRight;
	            m_rgrcDetected[i].bottom = rgRoi[i].m_nBottom;

	            //Venus_OutputDebug(szMsg);
	        }
			/*sprintf( szMsg, "GetSync Plate count:%d, Roi count:%d, avgY:%d\n",
					nPlateCount,
	                nRoiCount,
					nAvgY);
			Venus_OutputDebug(szMsg);*/
	...
	DWORD32 dwOneFrameTime(Venus_GetSystemTick());
	...
	if (pParam->fIsCaptureImage)
	{
		...
		{
		CRect rc(
			m_cRecogSnapArea.DetectorAreaLeft, 
			m_cRecogSnapArea.DetectorAreaTop, 
			m_cRecogSnapArea.DetectorAreaRight, 
			m_cRecogSnapArea.DetectorAreaBottom
			);
		return RecogSnapImg(
			pParam,
			pProcessData,
			...
			imgSnapFrame,
			rc, 
			m_cRecogSnapArea.nDetectorMinScaleNum,
			m_cRecogSnapArea.nDetectorMaxScaleNum
			);
		}
	}

	HV_COMPONENT_IMAGE imgCalibratedFrame; // 校正后的图像
	imgCalibratedFrame.nImgType = HV_IMAGE_YUV_422;

	if( m_nLightCount > 0 )
	{
		CheckTrafficLight(&imgSnapFrame);
	}
...
//	BOOL fCalibrateOK = m_fEnablePreCalibrate && (S_OK == m_pPretreatment->PreCalibrate(imgCurFrame, &imgCalibratedFrame));
//	bool fCalibrateOK = false;
//	if ( fCalibrateOK )
//	{
//		pImage->SetImageContent(imgCalibratedFrame);
//	}
//	else
//	{
		imgCalibratedFrame = imgSnapFrame;
//	}
	...
	{
		rcTrackArea = rcVideoDet;
	}


	for (int i = 0; i < m_cTrackInfo; i++)
	{
		CTrackInfo *pTrack=m_rgTrackInfo + i;
		pTrack->m_fObservationExists = false;

		pTrack->m_fPredictCarArrive = false;

		if (pTrack->m_cPlateInfo>=CTrackInfo::s_knMaxPlateInfo)
		{
			//保留最后3帧结果，用来预测下一帧位置。
			pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 4] = pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 3];
			pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 3] = pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 2];
			pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 2] = pTrack->m_rgPlateInfo[CTrackInfo::s_knMaxPlateInfo - 1];
			pTrack->m_cPlateInfo = CTrackInfo::s_knMaxPlateInfo - 1;
		}
	}
	...
	{
		// 不使用视频检测
		HV_DebugInfo( DEBUG_STR_TRACK_FILE, "---- FullSearch ----\n");
		nSearchNum = 1;
	}
	else
	{

		nDetectorTime = Venus_GetSystemTick();
		//移动物体检测

		//add by: qinyj for: 位置修正.新
		HV_COMPONENT_IMAGE imgCorpedVideoDetect;
		CropImage(imgCalibratedFrame, rcVideoDet, &imgCorpedVideoDetect);
		bool fLightIsNight = false;
		if (m_LightType == NIGHT)
		{
			fLightIsNight = true;
		}

        m_objDetectInfo.piRectCount = &m_cDetected;
        m_objDetectInfo.piRectEPCount = &m_cScanEP;
        m_objDetectInfo.pRects = m_rgrcDetected;
        m_objDetectInfo.pEPRects = m_rgrcScanEP;
        m_objDetectInfo.iMaxCount = s_knMaxObjCandidate;
        m_objDetectInfo.iDetectKind = 0;
        m_objDetectInfo.piRectModelCount = &m_cCarTail;
        m_objDetectInfo.pModelRects = m_rgrcCarTail;
        m_objDetectInfo.pVideoDetArea = &rcVideoDet;
        m_objDetectInfo.pOffSet = &(rcVideoDet.TopLeft());
        m_objDetectInfo.iSrcH = m_nHeight;
        m_objDetectInfo.iSrcW = m_nWidth;
        HRESULT hr = S_OK;
        if (1 == m_iNoPlateDetMode)
        {
        	//strcat(g_szDebugInfo, "Begin...\n");
            //DWORD32 dwBeginTick = GetSystemTick();
            hr = m_rgpObjectDetector[pParam->iVideoID]->ProcessEx(
			rcVideoDet.TopLeft(), &imgCorpedVideoDetect,
                fLightIsNight, m_objDetectInfo);
			if (m_cScanEP > s_knMaxObjCandidate)
			{
				m_cScanEP = s_knMaxObjCandidate;
			}

			//char szTmp[32] = {0};
			//sprintf(szTmp, "================m_cDetected:%d\n", m_cDetected);
            //strcat(g_szDebugInfo, szTmp);
            //HV_Trace("Perpare fg detect time = %d\n", GetSystemTick() - dwBeginTick);          
        }
        else
        {
			m_iLightDetected = 0;
			ObjDetInfo objDetInfo;
			objDetInfo.videoDet = &rcVideoDet;
			objDetInfo.piRectLightCount = &m_iLightDetected;
			objDetInfo.pLightRects = m_rcLightDetected;
    		hr = m_rgpObjectDetector[pParam->iVideoID]->Process(
    			rcVideoDet.TopLeft(), &imgCorpedVideoDetect,
    			m_rgrcDetected, s_knMaxObjCandidate, &m_cDetected, fLightIsNight, objDetInfo);
        }
		if(S_OK == hr)
		{	//根据当前视频平均亮度判断白天黑夜
			RTN_HR_IF_FAILED(m_rgpObjectDetector[pParam->iVideoID]->GetAvgGrey(m_iAvgY));
			nSearchNum = m_cDetected;
		}
		nDetectorTime = Venus_GetSystemTick() - nDetectorTime;
	...
	int nRegionTime = Venus_GetSystemTick();
	int nRegCount = 0;

	//车牌跟踪
	for (int iRegionIndex = 0; iRegionIndex < nSearchNum; iRegionIndex++)
	{
		CRect rcSearchRegion;
		if (m_nVideoDetMode != NO_VIDEODET)
		{
			if (!rcSearchRegion.IntersectRect(rcFullSearch, m_rgrcDetected[iRegionIndex]))
			{
				continue;
			}
		}
		...
		imgSearchRegion.nImgType = HV_IMAGE_YUV_422;

		RTN_HR_IF_FAILED(CropImage(imgCalibratedFrame, rcSearchRegion, &imgSearchRegion));

		DWORD32 dwTickCount=Venus_GetSystemTick();

		CItgArea* pItgArea = m_cItgArea.IsCalcItg()?&m_cItgArea:NULL;

		DWORD32 dwDetFrameTime(Venus_GetSystemTick());
		// 根据当前视频平均亮度判断白天黑夜
		if (m_nVideoDetMode == NO_VIDEODET)
		{
		...
		pProcessRespond->iFrameAvgY = m_iAvgY;
		//HV_DebugInfo( DEBUG_STR_OTHER_FILE, "FullScanTime = %d\n", GetSystemTick()-dwTickCount);

//		HV_DebugInfo( DEBUG_STR_TIME_FILE, "FullScanTime = %d\n", GetSystemTick()-dwTickCount);
//		HV_DebugInfo( DEBUG_STR_TRACK_FILE, "FullScanResultCount = %d\n", MultiROI.dwNumROI);

//		sprintf( szMsg, "FullScanTime = %d, plate %d\n", GetSystemTick()-dwTickCount, MultiROI.dwNumROI);
//		Venus_OutputDebug(szMsg);

		CheckCarArrive(pProcessRespond, pParam->iDiffTick);
		nRegCount += MultiROI.dwNumROI;
		...
		for (int i=0; i<MultiROI.dwNumROI; i++)
		{
			...
			RECTA *pROI = MultiROI.rcROI+i;
			CRect rcCurPlate(*pROI);
			rcCurPlate+=rcSearchRegion.TopLeft();

			GetInflateRect(rcCurPlate, pROI->color, pROI->nPlateType, m_nHeight);
			if (!rcCurPlate.IntersectRect(&rcCurPlate, &rcMax))
			{
				continue;
			}
			if (m_cfgParam.cTrapArea.fEnableDetAreaCtrl)
			{
				if (!IsContainOfTrap(m_cfgParam.cTrapArea, rcCurPlate))
				{
					continue;
				}
			}
			...
			// 识别，并将结果放入到rgPlates中
			HV_COMPONENT_IMAGE imgPlate;
			imgPlate.nImgType = HV_IMAGE_YUV_422;
			...
			{
				imgPlateRotateY.Create(HV_IMAGE_YUV_422, rcCurPlate.Width(), rcCurPlate.Height());
				imgPlateRotateY.CropAssign(imgCalibratedFrame, rcCurPlate);
				imgPlate = imgPlateRotateY;
			}
			else
			{
				RTN_HR_IF_FAILED(CropImage(imgCalibratedFrame, rcCurPlate, &imgPlate));
			}

			HV_DebugInfo( DEBUG_STR_TRACK_FILE,
								"RecognizePlate: (%d, %d) -> (%d, %d)\n",
								rcCurPlate.left, rcCurPlate.top,
								rcCurPlate.right, rcCurPlate.bottom
							);

			PlateInfo plateNew_Default, plateNew_Yellow, plateNew_Blue;
			plateNew_Default.color = pROI->color;
			plateNew_Default.nPlateType = pROI->nPlateType;
			plateNew_Default.rcPos = rcCurPlate;
			plateNew_Default.iLastDetWidth = pROI->right-pROI->left;
			plateNew_Default.nFrameNo = pParam->dwFrameNo;
			plateNew_Default.dwFrameTime = pParam->dwImageTime;
			plateNew_Default.nVariance = pROI->nVariance;
			plateNew_Default.fReRecog = TRUE;

			plateNew_Yellow.color = PC_YELLOW;
			plateNew_Yellow.nPlateType = PLATE_UNKNOWN;
			plateNew_Yellow.rcPos = rcCurPlate;
			plateNew_Yellow.iLastDetWidth = pROI->right-pROI->left;
			plateNew_Yellow.nFrameNo = pParam->dwFrameNo;
			plateNew_Yellow.dwFrameTime = pParam->dwImageTime;
			plateNew_Yellow.nVariance = pROI->nVariance;
			plateNew_Yellow.fReRecog = TRUE;
			
			plateNew_Blue.color = PC_BLUE;
			plateNew_Blue.nPlateType = PLATE_NORMAL;
			plateNew_Blue.rcPos = rcCurPlate;
			plateNew_Blue.iLastDetWidth = pROI->right-pROI->left;
			plateNew_Blue.nFrameNo = pParam->dwFrameNo;
			plateNew_Blue.dwFrameTime = pParam->dwImageTime;
			plateNew_Blue.nVariance = pROI->nVariance;
			plateNew_Blue.fReRecog = TRUE;

			PlateInfo *pPlateInfo;
			...
			if(baTempImg != NULL)
			{
				...
				int iIsWhiteChar = ( pROI->color == PC_BLACK ) || ( pROI->color  == PC_BLUE )|| (pROI->color  == PC_GREEN);
				IMAGE_Gray2BinNew( iIsWhiteChar, baTempImg,  imgPlate ,-1);
				int nColor = 0;
				CalcPlateColor(imgPlate,baTempImg,nColor);
				if(pROI->nPlateType == PLATE_NORMAL && (nColor == 1 || nColor == 6))  
				{
					plateNew_Default.fReRecog = FALSE;   
				}
				if(pROI->nPlateType == PLATE_UNKNOWN && nColor == 2)   
				{
					plateNew_Default.fReRecog = FALSE;              
					plateNew_Default.nPlateType = PLATE_POLICE;
				}
				if(pROI->nPlateType == PLATE_UNKNOWN && (nColor == 1 || nColor == 5))  
				{
					plateNew_Default.color = PC_LIGHTBLUE;
				}
			}
		
			CPersistentComponentImage imgRectified, imgRectifiedTemp;
			HRESULT hrRecog =  RecognizePlate(
								imgPlate, 
								plateNew_Default,
								imgRectified
								);
			RTN_HR_IF_FAILED(hrRecog);
			if(hrRecog != S_OK )
			{
				continue;
			}

			pPlateInfo = &plateNew_Default;

			if ( (pROI->nPlateType != PLATE_DOUBLE_YELLOW)
				&& (pROI->nPlateType != PLATE_DOUBLE_GREEN)
				&& (pROI->nPlateType != PLATE_DOUBLE_MOTO)
				&& (plateNew_Default.nPlateType != PLATE_WJ)
				&& (plateNew_Default.nPlateType != PLATE_DOUBLE_WJ)
				&& (plateNew_Default.fReRecog)                             //重识别
				&& ( plateNew_Default.fltTotalConfidence * 100 ).ToInt() < 15
				)
			{
				if (pROI->nPlateType == PLATE_NORMAL)
				{
						hrRecog =  RecognizePlate(
							imgPlate, 
							plateNew_Yellow,
							imgRectifiedTemp
							);
					...
					if (plateNew_Yellow.fltTotalConfidence > plateNew_Default.fltTotalConfidence 
						&&plateNew_Yellow.fltTotalConfidence.ToFloat() > 0.25)
					{
						pPlateInfo = &plateNew_Yellow;
						imgRectifiedTemp.Detach(imgRectified);
					}
				}
				else //pROI->nPlateType == PLATE_UNKNOWN
				{
					...
					if (plateNew_Blue.fltTotalConfidence > plateNew_Default.fltTotalConfidence)
					{
						pPlateInfo = &plateNew_Blue;
						imgRectifiedTemp.Detach(imgRectified);
					}
				}
			}

			if (pPlateInfo->fltTotalConfidence < m_pParam->g_fltThreshold_BeObservation)
			{
				continue;
			}

			if(CTrackInfo::m_fEnableRecgCarColor)
			{
				BOOL fIsNight(FALSE);
				//LockTracker();
				RTN_HR_IF_FAILED(RecognizeCarColour(imgCalibratedFrame,	*pPlateInfo,m_iAvgY, fIsNight));
				//UnLockTracker();
			}
			...
			for (int j = 0; j < m_cTrackInfo; j++)
			{
				...
				if (pTrack->m_iVideoID != pParam->iVideoID ||
					pTrack->m_fObservationExists)
					continue;
				if (pTrack->m_State == tsNew ||
					pTrack->m_State == tsMoving)
				{
					CRect rcPredict = pTrack->PredictPosition(pParam->dwImageTime, m_pParam->m_iDetectReverseEnable, m_pScaleSpeed);
					int iSimilarity = pTrack->Similarity(*pPlateInfo);
					if (iSimilarity > iMaxSimilarity &&
						rcPredict.IntersectsWith(pPlateInfo->rcPos))
					{
						iMaxSimilarity = iSimilarity;
						iMaxIndex = j;
					}
				}
			}
			//如果找到相似的跟踪则确认，否则建立新跟踪。
			int nThreSim = 4;
			...
			if (
				pPlateInfo->nPlateType == PLATE_WJ ||
				pPlateInfo->nPlateType == PLATE_DOUBLE_MOTO ||
				pPlateInfo->nPlateType == PLATE_DOUBLE_YELLOW)
			{
				nThreSim--;
			}
			...
			// zhaopy
			/*
			char szPlateTemp[100];
			memset(szPlateTemp, 0, 100);
			pPlateInfo->GetPlateNameAlpha(szPlateTemp);
			sprintf(szMsg,"<process>plate:%s.", (char*)szPlateTemp);
			Venus_OutputDebug(szMsg);
			*/

			// 收费站如果车牌达到3/7的检测区域高度，则让其强制更新，避免底部形变相似度不高造成多检
			bool fForceUpdate = false;
			if (GetCurrentParam()->g_PlateRcogMode == PRM_TOLLGATE
					&& m_pParam->g_MovingDirection == MD_TOP2BOTTOM
					&& pPlateInfo->rcPos.bottom > m_rgTrackInfo[i].LastInfo().rcPos.bottom)
			{
				...
				if (iMaxSimilarity > 0 && nMoveH * 7 > nDetH * 3 && m_rgTrackInfo[i].m_cPlateInfo > 2)
				{
					fForceUpdate = true;
				}
			}

			if (iMaxSimilarity > nThreSim || fForceUpdate)
			{
				pTrack = m_rgTrackInfo + iMaxIndex;
				pTrack->m_fObservationExists = true;

				RTN_HR_IF_FAILED(ProcessTrackState(pTrack, rcTrackArea, *pPlateInfo, imgCalibratedFrame, imgRectified, imgSnapFrame, imgCapFrame, pParam, pProcessRespond));
				continue;
			}

//			// 相近位置可信度不高，且在底部的车牌，不让其建立新跟踪
//			if (GetCurrentParam()->g_PlateRcogMode == PRM_TOLLGATE
//					&& m_pParam->g_MovingDirection == MD_TOP2BOTTOM
//					&& pPlateInfo->rcPos.bottom > m_iCarArrivedPos * m_nHeight / 100
//					)
//			{
//				Venus_OutputDebug("Filter MutiDet\n");
//				continue;
//			}

			if (pPlateInfo->fltTotalConfidence>m_pParam->g_fltThreshold_StartTrack)
			{
				//在判断车牌是否在梯形蚰?不在梯形区域内的车牌不建立跟踪
				//weikt 2011-7-20 
				if (m_cfgParam.cTrapArea.fEnableDetAreaCtrl)
				{
					if (!IsContainOfTrap(m_cfgParam.cTrapArea, rcCurPlate))
				...
				if (m_cTrackInfo<s_knMaxTrackInfo)
				{
					CTrackInfo& TrackInfo=m_rgTrackInfo[m_cTrackInfo++];
					...
					CRect rcVideoDet;
					RTN_HR_IF_FAILED(MapParamRect(rcVideoDet, rcMax, m_rcVideoDetArea));
					rectIntersect.IntersectRect(pPlateInfo->rcPos, &rcVideoDet);
					if(rectIntersect != pPlateInfo->rcPos)
					{
						//HV_Trace(1, "IntersectRect ...\n");
						CropImage(imgSnapFrame,rectIntersect,&imgPlate);
						pPlateInfo->rcPos = rectIntersect;
					}
					//weikt end

					RTN_HR_IF_FAILED(TrackInfo.NewTrack(
						pParam,
						imgPlate,
						*pPlateInfo,
						m_iLastLightStatus,
					...
					for (int j = 0; j < m_cTrackInfo; j++)
					{
						if (&m_rgTrackInfo[j] == &TrackInfo ||
							m_rgTrackInfo[j].m_iVideoID == pParam->iVideoID ||
							!m_rgTrackInfo[j].m_fVoted)
							continue;
						if (m_rgTrackInfo[j].IsTwins(TrackInfo))
							TrackInfo.m_fVoted = true;
					}
				}
				else
				{
					HV_DebugInfo(DEBUG_STR_ERROR_FILE, "ERROR:TrackInfo FULL!\n" );

					int iMinConfidentTrack(0);
					for (int j=1; j<m_cTrackInfo; j++)
					{
						if (m_rgTrackInfo[iMinConfidentTrack].TrackScore()>
							m_rgTrackInfo[j].TrackScore())
						{
							iMinConfidentTrack = j;
						}
					}
					CTrackInfo& TrackInfo = m_rgTrackInfo[iMinConfidentTrack];
					if (TrackInfo.m_fltMaxConfidence < pPlateInfo->fltTotalConfidence)
					{
						TrackInfo.Clear(pProcessRespond);
	...
	nRegionTime = Venus_GetSystemTick() - nRegionTime;



	int nOtherTime = Venus_GetSystemTick();
	int nSecondRecog = Venus_GetSystemTick();
	int iDoCount = 0;
	// Process each existing tracking
	for (int i=0; i<m_cTrackInfo; i++)
	{
		CTrackInfo *pTrack=m_rgTrackInfo+i;
		...
		if (pTrack->m_iVideoID != pParam->iVideoID ||
			...
			pTrack->m_State == tsTemTrack ||				// 在模版匹配跟踪状态，不进行车牌检测
		...
		bool fDelete = false;
		int iStopPos = (CTrackInfo::m_iStopLinePos < 55) ? CTrackInfo::m_iStopLinePos : 55;

		if(pTrack->m_cPlateInfo > 4)
		{
			if((pTrack->LastInfo().rcPlatePos.Width() < 65)
				||( pTrack->LastInfo().rcPos.CenterPoint().y < (iStopPos * CTrackInfo::m_iHeight / 100)) )
			{
				if( GetCurrentParam()->g_PlateRcogMode == PRM_ELECTRONIC_POLICE ) fDelete = true;
			}
		}
		
		// zhaopy
		if( CTrackInfo::m_iWidth < 1900 )
		{
			fDelete = false;
		}
		...
		CRect rcPredict = pTrack->PredictPosition(pParam->dwImageTime, m_pParam->m_iDetectReverseEnable, m_pScaleSpeed);
		//CItgArea* pItgArea = pTrack->GetItgArea();
		CItgArea* pItgArea = NULL;

		if (rcPredict.IntersectRect(&rcPredict, &rcTrackArea) && !fDelete)
		{
			iDoCount++;
			HV_COMPONENT_IMAGE imgTrackRegion;
			imgTrackRegion.nImgType = HV_IMAGE_YUV_422;
			RTN_HR_IF_FAILED(CropImage(imgCalibratedFrame, rcPredict, &imgTrackRegion));

			//不知道车速和方向的时候使用快速检测
			//BOOL fFastMode = (pTrack->m_cPlateInfo <= 1 ? TRUE : FALSE);
			BOOL fFastMode = TRUE;

			//车辆逆行的时候修改车牌Scale预测参数
			int nSubscanPredictMinScale = m_pParam->g_nSubscanPredictMinScale;
			int nSubscanPredictMaxScale = m_pParam->g_nSubscanPredictMaxScale;
			if (pTrack->m_fReverseRun)
			{
				m_pParam->g_nSubscanPredictMinScale = -nSubscanPredictMaxScale;
				m_pParam->g_nSubscanPredictMaxScale = -nSubscanPredictMinScale;
			}
			
			DWORD32 dwTickCount = Venus_GetSystemTick();
			switch (pTrack->LastInfo().color)
			{
			case PC_BLUE:
			case PC_BLACK:
				RTN_HR_IF_FAILED(m_pPlateDetector->ScanPlatesComponentImage(
			...
			case PC_WHITE:
			case PC_LIGHTBLUE:
				if ((pTrack->LastInfo().nPlateType != PLATE_DOUBLE_YELLOW)
					&& (pTrack->LastInfo().nPlateType != PLATE_DOUBLE_MOTO))
				{
					// 单黄牌模型
					RTN_HR_IF_FAILED(m_pPlateDetector->ScanPlatesComponentImage(
			...
			HV_DebugInfo( DEBUG_STR_TIME_FILE, "SubScanTime = %d\n", Venus_GetSystemTick()-dwTickCount);

			m_pParam->g_nSubscanPredictMinScale = nSubscanPredictMinScale;
			m_pParam->g_nSubscanPredictMaxScale = nSubscanPredictMaxScale;
		}
		else
		{
			MultiROI.dwNumROI = 0;
		...
		pTrack->m_fObservationExists = (MultiROI.dwNumROI>0);

		PlateInfo plateObserved;
		CPersistentComponentImage imgBestRectified;
		int iSimilarityObserved = 0;
		if (pTrack->m_fObservationExists)
		{
 			for (int i=0; i<MultiROI.dwNumROI; i++)
			{
				...
				imgObservedPlate.nImgType = HV_IMAGE_YUV_422;
				...
				{
					RTN_HR_IF_FAILED(CropImage(imgCalibratedFrame, rcCurPlate, &imgObservedPlate));
				}
				...
				plateInfo.rcPos = rcCurPlate;
				plateInfo.color = pROI->color;
				plateInfo.nPlateType = pROI->nPlateType;
				plateInfo.iLastDetWidth = pROI->right-pROI->left;
				plateInfo.nFrameNo = pParam->dwFrameNo;
				plateInfo.dwFrameTime = pParam->dwImageTime;
				plateInfo.nVariance = pROI->nVariance;
				plateInfo.nInfoType = PI_LPR;
				...
				if (plateInfo.fltTotalConfidence < m_pParam->g_fltThreshold_BeObservation)
				{
					continue;
				}
				...
				if(CTrackInfo::m_fEnableRecgCarColor)
				{
					BOOL fIsNight(FALSE);
					//LockTracker();
					RTN_HR_IF_FAILED(RecognizeCarColour(imgCalibratedFrame,	plateInfo, m_iAvgY,fIsNight));
					//UnLockTracker();
				}

				int iSimilarity = pTrack->Similarity(plateInfo);
				if ((iSimilarity > 3 || m_pParam->g_PlateRecogSpeed == PR_SPEED_NORMAL)
					&& iSimilarity > iSimilarityObserved)
				{
					RTN_HR_IF_FAILED(imgRectified.Detach(imgBestRectified));
					plateObserved=plateInfo;
					iSimilarityObserved = iSimilarity;
				}
			}
			pTrack->m_fObservationExists = (iSimilarityObserved > 0);

		}
		RTN_HR_IF_FAILED(ProcessTrackState(pTrack, rcTrackArea, plateObserved, imgCalibratedFrame, imgBestRectified, imgSnapFrame, imgCapFrame, pParam, pProcessRespond));
	} //end for

	//检测CarArrive
	CheckCarArrive(pProcessRespond, pParam->iDiffTick);

	nSecondRecog = Venus_GetSystemTick() - nSecondRecog;

	nOtherTime = Venus_GetSystemTick();
	...
	if (m_pParam->m_fUseTemplet)
	{
		...
		for (int i = 0; i < m_cTrackInfo; i++)
		{
			...
			pTrack->m_fTempTrackExists = false;				// 模版匹配设置初值
			...
			if (pTrack->m_fObservationExists)
			{
				...
				if (m_pParam->m_fUsedTrafficLight)
				{
					...
					if(rcLast.Width() > 80)
					{
						rcLast.right -= 20;
						rcLast.top -= 5;
						rcLast.top = rcLast.top < 0?0:rcLast.top;
					}
					// 更新最小Y坐标
					if (rcLast.bottom < pTrack->m_nMinYPos)
					{
						pTrack->m_nMinYPos = rcLast.bottom;
					}
				}
				continue;				// 对于有跟踪的,跳过
			}

			if (!pTrack->m_fhasTemple)
			{
				continue;				// 当前没有模板的，则跳过
			}

			if ((m_pParam->m_fUsedTrafficLight) && IsCarStop(pTrack))
			{
				// 红绿灯条件下车辆停止，则停止模板匹配
				continue;
			}
			...
			// 跟踪失败,则对本跟踪使用模版匹配
			CRect rcPredict = pTrack->PredictPosition(pParam->dwImageTime, m_pParam->m_iDetectReverseEnable, m_pScaleSpeed);
			rcPredict.IntersectRect(rcPredict, rcMax);

			if ((rcPredict.left < 0 || rcPredict.right < 0 || rcPredict.top < 0 || rcPredict.bottom < 0
				|| rcPredict.right > m_nWidth ||  rcPredict.bottom > m_nHeight)
				|| rcPredict.IsRectEmpty())
			{
				// 预测失败,停止模板匹配
				continue;
			}

			CRect rcNext(-1, -1, -1, -1);
			HV_COMPONENT_IMAGE imgPre;
			CropImage(imgSnapFrame, rcPredict, &imgPre);
            //模版匹配之前进行图像格式转换,2012-03-19
			CPersistentComponentImage imgYUVTemp;
			if (imgPre.nImgType != HV_IMAGE_YUV_422)
			{
				imgYUVTemp.Create(HV_IMAGE_YUV_422, imgPre.iWidth, imgPre.iHeight);
				imgYUVTemp.Convert(imgPre);
				imgPre = (HV_COMPONENT_IMAGE)imgYUVTemp;
			}
			

			if (m_LightType == DAY)
			{
				pTrack->m_pObj->GetBestMatch(&imgPre, &rcNext, 
					true, m_fltHMThreld, false, 
					m_HMDelay, m_iMaxMatchDiff, true);
			} else {
				pTrack->m_pObj->GetBestMatch(&imgPre, &rcNext, 
					true, m_fltHMThreldNight, false, 
					m_HMDelayNight, m_iMaxMatchDiff, false);
			}
			...
			// 匹配成功，调整匹配位置到全图位置
			rcNext.left += rcPredict.left;
			rcNext.right += rcPredict.left;
			rcNext.top += rcPredict.top;
			rcNext.bottom += rcPredict.top;
			rcNext.IntersectRect(rcNext, rcPredict);

			// 模板匹配成功，需判断匹配位置是否符合轨迹
			// 解决红绿灯状态下的模版匹配乱跳问题
			if (m_pParam->m_fUsedTrafficLight)
			{
				//if ((rcNext.bottom - pTrack->m_nMinYPos) > 0)
				if ((rcNext.bottom - pTrack->m_nMinYPos) > 25)
				{
					continue;						// 模板匹配结果不可信，结束模板匹配
				}

				//解决乱跳的问题
				int nYMove =  abs(rcNext.CenterPoint().y - pTrack->LastInfo().rcPos.CenterPoint().y);
				int nXmove =  abs(rcNext.CenterPoint().x - pTrack->LastInfo().rcPos.CenterPoint().x);
				if( nYMove > pTrack->m_nMaxYPos || nXmove > pTrack->m_nMaxXPos )
				{
  //					HV_Trace("\n<Lost>x: %d,%d y:%d,%d \n", nXmove, pTrack->m_nMaxXPos, nYMove, pTrack->m_nMaxYPos);
					continue;
				}

				if (!rcTrackArea.Contains(rcNext))
				{
					continue;						// 开出了跟踪区域，也不必继续跟踪
				}

				// 本次模板匹配成功，更新最小位置
				if (rcNext.bottom < pTrack->m_nMinYPos)
				{
					pTrack->m_nMinYPos = rcNext.bottom;
				}
			}

			// 将模版匹配加入TrackInfo
			PlateInfo plateTem;						// 模版匹配产生的无牌车信息
			plateTem = pTrack->LastInfo();

			plateTem.nInfoType = PI_TEM;
			plateTem.rcPos = rcNext;

			plateTem.nFrameNo = pParam->dwFrameNo;
			plateTem.dwFrameTime = pParam->dwImageTime;

			pTrack->m_rgPlateInfo[pTrack->m_cPlateInfo++] = plateTem;

			pTrack->m_fTempTrackExists = true;		// 设置模版匹配成功标志
		}
	}
	...
	//	在红绿灯场景下，X和Y的偏移会比较小。
	//	且跟踪过了55%就不做此处理.
	float fltOffsetX, fltOffsetY;
	if( m_pParam->m_fUsedTrafficLight )
	{
		fltOffsetX = 1.8f;
		fltOffsetY = 1.1f;
	}
	else if (m_pParam->g_PlateRcogMode == PRM_HIGHWAY_TAIL)	// Li 调整车尾阈值
	{
		fltOffsetX = 2.0f;
		fltOffsetY = 1.1f;
	}
	...
	for (int i=0; i<m_cTrackInfo; i++)
	{
		// 黄学?2011-1-7)，增对WJ牌出两次牌的情况，将m_nMissingTime的判断由0调整为1，并设置其他判断条件
		if (!((m_rgTrackInfo[i].m_State == tsNew) ||
			(m_rgTrackInfo[i].m_State == tsMoving)) ||
			(m_rgTrackInfo[i].m_nMissingTime > 1))
		{
			continue;
		}

		// 黄学雷(2011-1-7)，加入的m_nMissingTime判断，加入卡口车头，在前面30%丢帧的限制
		if(m_rgTrackInfo[i].m_nMissingTime == 1)
		{/*
			if (m_pParam->g_PlateRcogMode != PRM_HIGHWAY_HEAD)
			{
				continue;
			}
		*/
			int nTempPer = m_rgTrackInfo[i].LastInfo().rcPos.bottom * 100;
			nTempPer /= m_nHeight;
		...
		for (int j=i+1; j<m_cTrackInfo; j++)
		{
			if ((!((m_rgTrackInfo[j].m_State == tsNew) ||
				(m_rgTrackInfo[j].m_State == tsMoving)) ||
				(m_rgTrackInfo[j].m_nMissingTime > 0)) ||
				m_rgTrackInfo[i].m_iVideoID != m_rgTrackInfo[j].m_iVideoID)
			{
				continue;
			}
			// li 跟踪帧数大于10帧的牌不能去除 只对卡口车尾
			if (m_rgTrackInfo[i].m_cPlateInfo > 10 && m_rgTrackInfo[j].m_cPlateInfo > 10
				&& m_pParam->g_PlateRcogMode == PRM_HIGHWAY_TAIL)
			{
				continue;
			}
			CRect rect1 = m_rgTrackInfo[i].LastInfo().rcPlatePos;
			CRect rect2 = m_rgTrackInfo[j].LastInfo().rcPlatePos;
			int iPlateWidth = (rect1.Width() + rect2.Width()) / 2;
			rect1 = m_rgTrackInfo[i].LastInfo().rcPos;
			rect2 = m_rgTrackInfo[j].LastInfo().rcPos;

			if( m_pParam->m_fUsedTrafficLight && (rect2.CenterPoint().y * 100 / m_nHeight ) < 55 )
			{
				continue;
			}

			if (m_pParam->g_fBestResultOnly ||
				(HV_ABS(rect1.CenterPoint().x - rect2.CenterPoint().x) < (int)((float)iPlateWidth * fltOffsetX) &&
				HV_ABS(rect1.CenterPoint().y - rect2.CenterPoint().y) < (int)((float)iPlateWidth * fltOffsetY)) )
			{
				// 黄学雷(2011-1-7),对MissingTime为1的单独判断
				if (m_rgTrackInfo[i].m_nMissingTime == 1)
				{
					// 替换位置只有1帧时，才应用这个修改
					if (m_rgTrackInfo[j].m_cPlateInfo == 1)
					{
						m_rgTrackInfo[i].m_State = tsWaitRemove;
					}
				}
				else if(!m_rgTrackInfo[i].m_bPreVote && !m_rgTrackInfo[j].m_bPreVote)
				{
					if (m_rgTrackInfo[i].TrackScore() > m_rgTrackInfo[j].TrackScore())
					{
						// TODO 这个不是最好的办法，红绿灯条件下的车牌重叠判断应该才用新的方法
						if (m_rgTrackInfo[j].m_State == tsTemTrack)
						{
							m_rgTrackInfo[j].m_State = tsTemTrackEnd;
						}
						else
						{
					...
					{
						if (m_rgTrackInfo[i].m_State == tsTemTrack)
						{
							m_rgTrackInfo[i].m_State = tsTemTrackEnd;
						}
						else
						{
							m_rgTrackInfo[i].m_State = tsWaitRemove;
					...
					{
					if(!m_rgTrackInfo[j].m_bPreVote || m_rgTrackInfo[i].TrackScore() > m_rgTrackInfo[j].TrackScore())
					{
						if (m_rgTrackInfo[j].m_State == tsTemTrack)
						{
							m_rgTrackInfo[j].m_State = tsTemTrackEnd;
					...
					else if(!m_rgTrackInfo[i].m_bPreVote || m_rgTrackInfo[j].TrackScore() > m_rgTrackInfo[i].TrackScore())
					{
						if (m_rgTrackInfo[i].m_State == tsTemTrack)
						{
							m_rgTrackInfo[i].m_State = tsTemTrackEnd;
	...
	if (m_pParam->m_fUseTemplet)
	{
		...
		for (int i = 0; i < m_cTrackInfo; i++)
		{
			...
			pTrack->m_fhasTemple = false;			// 设置没有模板标志

			// 建立或者更新模版
			if (IsCanFoundTemple(pTrack))
			{
				CRect rcObjBegin = pTrack->LastInfo().rcPos;					// 找到当前车牌坐标

				//更新当前的车牌坐标的的位置 weikt 2011-5-30
				if(m_pParam->m_fUsedTrafficLight)
				{
					if( (rcObjBegin.right - rcObjBegin.left) / 2 > (rcObjBegin.bottom - rcObjBegin.top) )
					{
						rcObjBegin.top -= ((rcObjBegin.right - rcObjBegin.left) / 2 - (rcObjBegin.bottom - rcObjBegin.top));
						rcObjBegin.top = rcObjBegin.top < 0?0:rcObjBegin.top;
					}
				}
				//end weikt 2011-5-30
				HRESULT hr = pTrack->UpdateObjTemple(imgSnapFrame, rcObjBegin);		// 更新模版
				if (SUCCEEDED(hr))
				{
					pTrack->m_fhasTemple = true;								// 设置有模板标志
				}
			}

			// 模版匹配跟踪状态
			if (pTrack->m_State == tsTemTrack)
			{
				//
				ProcessTemTrackState(pTrack, rcTrackArea);
				if ((!pTrack->m_fTempTrackExists) || (!pTrack->m_fhasTemple))
				{
					pTrack->m_State = tsTemTrackEnd;
				}
				HV_COMPONENT_IMAGE imgTmp1, imgTmp2;
				imgTmp1.nImgType = HV_IMAGE_YUV_422;
				imgTmp2.nImgType = HV_IMAGE_YUV_422;
				//触发抓拍
				...
				RTN_HR_IF_FAILED(pTrack->UpdatePlateImage(
					imgTmp1, pParam, m_iLastLightStatus, m_iCurLightStatus, pProcessRespond
					));
			}
		} // for
	}

	RTN_HR_IF_FAILED(ShowTracks(pParam, pProcessData, pProcessRespond));
	if (m_nVideoDetMode == USE_VIDEODET || GetCurrentParam()->g_PlateRcogMode == PRM_TOLLGATE)
	{
		RTN_HR_IF_FAILED(TrackingObj(pParam, imgSnapFrame, pProcessRespond));
		if (1 == m_iNoPlateDetMode)
		{
		    TrackingObjByTem(pParam, imgSnapFrame, pProcessRespond);
		}
		RTN_HR_IF_FAILED(ShowObjTracks(pParam, pParam->iVideoID, pProcessRespond));
	}
	RTN_HR_IF_FAILED(RemoveInvalidTracks(pProcessRespond));

	dwOneFrameTime = Venus_GetSystemTick() - dwOneFrameTime;

	if( dwOneFrameTime > 150 )
	{
	//	HV_Trace("OutTimeFrame:%d,SreachTime:%d,Count:%d,Region:%d,OtherTime:%d\n", dwOneFrameTime, nDetectorTime, nSearchNum, nRegionTime, nOtherTime);
	}

	dwTotalFrameTime += dwOneFrameTime;
	dwFrameNum++;
	if ( dwFrameNum > 10000 )
	{
		dwFrameNum = 0;
	...
	HV_DebugInfo( DEBUG_STR_OTHER_FILE, "from_1092_DBL----OneFrameTime = %d, TotalTime = %d, Num = %d\n", dwOneFrameTime, dwTotalFrameTime, dwFrameNum );	
	...
	for(int i = 0; i < m_cTrackInfo; i++)
	{
		(m_rgTrackInfo+i)->m_cItgArea.SetRect(0,0,0,0,FALSE);
	}

	GetLightType(&pProcessRespond->cLightType);
	...
	if (GetTrackCount(&dwTrackCount) != S_OK)
	{
		dwTrackCount = 0;
	}
	/*
	pProcessRespond->cTrackRectInfo.dwTrackCount = MIN(
		dwTrackCount, 
		sizeof(pProcessRespond->cTrackRectInfo.rgTrackRect) / sizeof(pProcessRespond->cTrackRectInfo.rgTrackRect[0])
		);
	*/
	int iMaxRect = sizeof(pProcessRespond->cTrackRectInfo.rgTrackRect) / sizeof(pProcessRespond->cTrackRectInfo.rgTrackRect[0]);
	pProcessRespond->cTrackRectInfo.dwTrackCount = 0;
	for (int i = 0, index = 0; i < iMaxRect - 1; i+=2, index++)
	{
		if( index >= (int)dwTrackCount ) break;
		if( S_FALSE == 
			GetPlatePos(
				index, 
				&(pProcessRespond->cTrackRectInfo.rgTrackRect[i]), 
				&(pProcessRespond->cTrackRectInfo.rgTrackRect[i+1])) )
		{
			i -= 2;
		}
		else
		{
			pProcessRespond->cTrackRectInfo.dwTrackCount += 2;
		}
	}

	DWORD32 dwEPObjCount = 0;
	GetEPObjCount(&dwEPObjCount);
	if (dwEPObjCount > 0)
	{
		int iLeft = pProcessRespond->cTrackRectInfo.dwTrackCount;
		DWORD32 dwIndex = 0;
		for (; iLeft<iMaxRect; ++iLeft)
		{
			if (S_OK != GetEPObjPos(dwIndex, &(pProcessRespond->cTrackRectInfo.rgTrackRect[iLeft])))
			{
				break;
			}
			++dwIndex;
			++pProcessRespond->cTrackRectInfo.dwTrackCount;
		}
	}

	if(dwTrackCount >= 10 || dwEPObjCount >= 10)	
	{
		//sprintf(g_szDebugInfo, "dwTrackCount=%d, dwEPObjCount=%d", dwTrackCount, dwEPObjCount);
	}


	int indexRed = 0;
	for(int i = pProcessRespond->cTrackRectInfo.dwTrackCount; i < iMaxRect; i++)
	{
		if( indexRed >= m_nRedLightCount ) break;
		pProcessRespond->cTrackRectInfo.rgTrackRect[pProcessRespond->cTrackRectInfo.dwTrackCount].left = m_rgRedLightRect[indexRed].left;
		pProcessRespond->cTrackRectInfo.rgTrackRect[pProcessRespond->cTrackRectInfo.dwTrackCount].top = m_rgRedLightRect[indexRed].top;
		pProcessRespond->cTrackRectInfo.rgTrackRect[pProcessRespond->cTrackRectInfo.dwTrackCount].right = m_rgRedLightRect[indexRed].right;
		pProcessRespond->cTrackRectInfo.rgTrackRect[pProcessRespond->cTrackRectInfo.dwTrackCount].bottom = m_rgRedLightRect[indexRed].bottom;
		pProcessRespond->cTrackRectInfo.dwTrackCount++;
		indexRed++;
	}
	//红灯坐标
	memset(pProcessRespond->rcRedLight, 0, sizeof(pProcessRespond->rcRedLight));
	for (int i = 0; i < m_nRedLightCount; i++)
	{
		pProcessRespond->rcRedLight[i].left =  m_rgRedLightRect[i].left;		
		pProcessRespond->rcRedLight[i].top =  m_rgRedLightRect[i].top * 2;
		pProcessRespond->rcRedLight[i].right =  m_rgRedLightRect[i].right;
		pProcessRespond->rcRedLight[i].bottom =  m_rgRedLightRect[i].bottom * 2;
	}
/*
	sprintf( szMsg, "<end>");
	if( strlen( szMsg ) + strlen( g_szDebugInfo ) < 255 )
	{
		strcat( g_szDebugInfo, szMsg );
	}
*/
/*
	nOtherTime = GetSystemTick() - nOtherTime;
	sprintf( szMsg, "<other,time:%d>",nOtherTime);
	if( strlen( szMsg ) + strlen( g_szDebugInfo ) < 255 )
	{
		strcat( g_szDebugInfo, szMsg );
	}
...
//	RTN_HR_IF_FAILED(pTrack->UpdateBestImage(pImage));
	...
	case tsMoving:
		...
		else
		{
			...
			pTrack->LastInfo().iLastCharHeight=-1;
			pTrack->LastInfo().iLastVscale=-1;
			BOOL fPredictInEndTrackArea = TRUE;//!rcTrackArea.IntersectsWith(pTrack->PredictPosition(pParam->dwImageTime, m_pParam->m_iDetectReverseEnable, m_pScaleSpeed));
...
HRESULT CTrackerImpl::SetFirstLightType(LIGHT_TYPE nLightType)
{
	...
	m_dwLastCarLeftTime = GetSystemTick();
	m_cTgTrackCtrl.SetFirstLightType(nLightType);
	return S_OK;
}
...
{
	if(m_fEnableScaleSpeed == FALSE || m_fEnableCarArriveTrigger == FALSE) return S_FALSE;
	...
	for(int i = 0; i < m_cTrackInfo; ++i)
	{
				...
				)
		{
			...
			//预测CarArrive的时间.
			float fltDistance = 0.f;//m_pScaleSpeed->CalcActureDistance(rcPos.CenterPoint(), CPoint(rcPos.CenterPoint().x, iPosY));
			//计算车辆到达触发延迟时间
			float fltCarSpeed(0.0f);
			float fltScaleOfDistance(1.0f);
