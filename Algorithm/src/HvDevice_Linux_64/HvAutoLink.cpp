...
{
    SOCKET_NUM *pNewSocket = (SOCKET_NUM*)lpParameter;
	SOCKET hNewSocket = pNewSocket->nNewSocket;
...
HRESULT SetNewLink(LPVOID lpParameter ,SOCKET hNewSocket )
{
	...
	{
		if (0 == pServer->prgProcessNewLinkThread[i])
		{
	...
	if (LISTEN_TYPE_RECORD == pServer->nMonitorType && NULL != phFreeThread)
	{
        ...
        SOCKET_NUM pNewSocket;
        pNewSocket.nNewSocket = hNewSocket;
		if (  pthread_create( &th,  NULL, ProcessRecordNewLinkThreadFunc, (LPVOID)&pNewSocket ) )
		{
			return E_FAIL;
		}
		*phFreeThread = th;
