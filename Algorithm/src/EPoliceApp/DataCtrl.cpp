...
{
    return g_pEPolice->ForceSend(dwVideoID);
}

HRESULT GetWorkModeInfo(SYS_INFO* pInfo)
{
    return(g_pEPolice->GetWorkModeInfo(pInfo));
}

HRESULT GetWorkModeList(SYS_INFO* pInfo)
{
    return(g_pEPolice->GetWorkModeList(pInfo));
}
...
HRESULT CEPolice::ForceSend(DWORD32 dwVideoID)
{
...
HRESULT CEPolice::GetWorkModeInfo(SYS_INFO* pInfo)
{
...
HRESULT CEPolice::GetWorkModeList(SYS_INFO* pInfo)
{
...
void CEPolice::OnImage(void* pContext, IMG_FRAME imgFrame)
{
    CEPolice * pThis = (CEPolice *)pContext;
...
HRESULT CEPolice::Run(void* pvParam)
{
    ...
    while (!m_fExit)
    {
        ...
        m_pSignalMatch->GetTrafficLightStatus(&cProcessEvent.nIOLevel);
        ...
        if (hr == S_OK)
        {
            ...
            m_pVideoGetter->SetLightType(cProcessEvent.cLightType, -1);
            if (g_iControllPannelWorkStyle == 1)
            {
                    ...
                    && (int)(GetSystemTick() - dwSendVideo) >= m_cModuleParams.cResultSenderParam.iVideoDisplayTime)
            {
                ...
                if (imgFrame.nImgType == HV_IMAGE_JPEG)
                {
                    ...
                    videoInfo.cVideoExtInfo.iShutter = m_cModuleParams.cResultSenderParam.iEddyType;
                    ...
                    memcpy(videoInfo.rcRedLight, cProcessEvent.rcRedLight, sizeof(videoInfo.rcRedLight));
...
HRESULT CEPolice::SetAWBFromLightType(LIGHT_TYPE emLightType)
{
...
HRESULT CEPolice::ScaleCarFlowRate()
{
...
)
{
    ...
    m_pResultSender->PutCarEnterLeaveFlag(0, 0, dwTimeLow, dwTimeHigh);*/
    return S_OK;
}

HRESULT CEPolice::CarLeft(
...
)
{
    ...
    //在这里转换红绿灯的时间
    LIGHT_REALTIME *lr = &pCarLeftInfo->cCoreResult.lrtRedStart;
    if (lr->dwLeftL)
    {
        ConvertTickToSystemTime(lr->dwLeftL, lr->dwLeftL, lr->dwLeftH);
    }
    if (lr->dwForwardL)
    {
        ConvertTickToSystemTime(lr->dwForwardL, lr->dwForwardL, lr->dwForwardH);
    }
    if (lr->dwRightL)
    {
        ConvertTickToSystemTime(lr->dwRightL, lr->dwRightL, lr->dwRightH);
    }
    if (lr->dwTurnL)
    {
        ConvertTickToSystemTime(lr->dwTurnL, lr->dwTurnL, lr->dwTurnH);
    }
...
HRESULT CEPolice::BuildPlateString(
    char* pszPlateString,
...
)
{
    ...
    //修正过滤不合格违章图片
    pCarLeftInfo->cCoreResult.iUnSurePeccancy = 0;
    RESULT_IMAGE_STRUCT* pResultImage = &(pCarLeftInfo->cCoreResult.cResultImg);
    if (  pResultImage->pimgBeginCapture == NULL
    ...
    {
        pCarLeftInfo->cCoreResult.iUnSurePeccancy = 1;
    }

    //违章否的仅输出一张图
    if(pCarLeftInfo->cCoreResult.iUnSurePeccancy == 1)
    {
    ...
    if (pResult)
    {
        ...
        if (m_cModuleParams.cResultSenderParam.fOutputAppendInfo)
        {
            ...
            if (pCarLeftInfo->cCoreResult.cResultImg.pimgBeginCapture)
            {
                //视频帧名
                strcpy(
                    szFrameName,
                    pCarLeftInfo->cCoreResult.cResultImg.pimgBeginCapture->GetFrameName()
                );
        ...
        if (g_iControllPannelWorkStyle == 1
        && m_cModuleParams.cCamCfgParam.iDynamicCfgEnable )
        {
            //当前补光灯脉宽级别
...
bool CEPolice::ThreadIsOk(int* piErrCode)
{
...
void CEPolice::SetModuleParams(const ModuleParams& cParam)
{
    m_cModuleParams = cParam;
}

void CEPolice::GetCarColor(CARLEFT_INFO_STRUCT *pCarLeftInfo, char *pszConf)
{
...
void CEPolice::GetCarType(PROCESS_IMAGE_CORE_RESULT *pImgCoreInfo, char *pszConf)
{
...
void CEPolice::GetEventDetInfo(CARLEFT_INFO_STRUCT *pCarLeftInfo, char *pszConf)
{
    bool fIsTurnArround = false;
    bool fIsOverSpeed = false;
    pszConf[0] = 0;
/*
    if (m_pEventChecker && m_pEventChecker->IsTurnAround(pCarLeftInfo))
    {
        fIsTurnArround = true;
    }
*/
    if (pCarLeftInfo->cCoreResult.fltCarspeed >= 0.1f
            && ((int)pCarLeftInfo->cCoreResult.fltCarspeed > m_cModuleParams.cResultSenderParam.iSpeedLimit))
    {
        fIsOverSpeed = true;
    }
    ...
    if ( fIsTurnArround )
    {
        //黄国超修改,2010-12-08,并注释掉下面的数据
        pCarLeftInfo->cCoreResult.ptType |= PT_ACROSS;
    }
    //违章类型中,超速违章等级最低
    if (fIsOverSpeed)
    {
        //黄国超修改，增加或操作,2010-12-08
        pCarLeftInfo->cCoreResult.ptType |= PT_OVERSPEED;
    }
    //是否要过滤违章不合格记录
    bool fIsUnSure = false;
    if (m_cModuleParams.cResultSenderParam.iFilterUnSurePeccancy == 1
            && pCarLeftInfo->cCoreResult.iUnSurePeccancy == 1)
    {
        fIsUnSure = true;
    }
    int nRoadNo = pCarLeftInfo->cCoreResult.nRoadNo;
    if(m_cModuleParams.cSignalMatchParam.iRoadNumberBegin)
    {
        nRoadNo = m_cModuleParams.cTrackerCfgParam.nRoadLineNumber - 2 - nRoadNo;
    }
    if (m_cModuleParams.cTrackerCfgParam.nOutputRoadTypeName && nRoadNo >=0 && nRoadNo < MAX_ROADLINE_NUM - 1)
    {
#define STRING_ROAD_TYPE "直行;左转;直行,左转;右转;直行,右转;左转,右转;直行,左转,右转;调头;直行,调头;左转,调头;直行,左转,调头;右转,调头;直行,右转,调头;左转,右转,调头;直行,左转,右转,调头;非机动车道;"
        CHvString strRoadType = m_cModuleParams.cTrackerCfgParam.rgcRoadInfo[nRoadNo].szRoadTypeName;
        CHvString szTmp = strRoadType;
        szTmp.MakeUpper();
        if (strRoadType.IsEmpty() || !strcmp(szTmp.GetBuffer(),"NULL"))
        {
            CHvString strTmp = STRING_ROAD_TYPE;
            int i = 0, j = 0;
            for (int pos = 0; j >= 0 && pos < m_cModuleParams.cTrackerCfgParam.rgcRoadInfo[nRoadNo].iRoadType;)
            {
                j = strTmp.Find(";", i);
                pos++;
                if (pos < m_cModuleParams.cTrackerCfgParam.rgcRoadInfo[nRoadNo].iRoadType)
                    i = j + 1;
            }
            if (j < 0)
                j = strTmp.GetLength();
            szTmp = strTmp.Mid(i, j - i);
            if (szTmp.IsEmpty())
                szTmp = "未知";
            strRoadType = CHvString("车道类型:") + szTmp;
        }
        else
            strRoadType = CHvString("车道类型:") + strRoadType;

        strcat(pszConf, strRoadType.GetBuffer());
        strcat(pszConf, "\n");
    }
    switch (pCarLeftInfo->cCoreResult.rtType)
    {
    case RT_FORWARD:
        strcat(pszConf, "行驶类型:直行\n");
        break;
    case RT_LEFT:
        strcat(pszConf, "行驶类型:左转\n");
        break;
    case RT_RIGHT:
        strcat(pszConf, "行驶类型:右转\n");
        break;
    case RT_TURN:
        strcat(pszConf, "行驶类型:调头\n");
        break;
    case RT_CONVERSE:
        strcat(pszConf, "行驶类型:逆行\n");
        break;
    default:
        strcat(pszConf, "行驶类型:未知\n");
        break;
    }
    //黄国超修改，注释掉原来的违章类型，通过或判断所有违章类型
    if (pCarLeftInfo->cCoreResult.ptType != PT_NORMAL)
    {
        if ( fIsUnSure )
        {
            strcat(pszConf, "违章:否");
        ...
        //按照优先级判断该输出哪种违章类型,黄国超修改,2010-12-08
        if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
        {
            CHvString str = m_cModuleParams.cResultSenderParam.szPeccancyPriority;
            ...
            while (i != -1 && j != -1)
            {
                ...
                if (j != -1)
                {
                    CHvString s = str.Mid(i, j-i);
                    if (s == (CHvString)"逆行"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_CONVERSE) == PT_CONVERSE)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_CONVERSE;
                        break;
                    }
                    if (s == (CHvString)"非机动车道"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_ESTOP) == PT_ESTOP)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_ESTOP;
                        break;
                    }
                    if (s == (CHvString)"闯红灯"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_RUSH) == PT_RUSH)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_RUSH;
                        break;
                    }
                    if (s == (CHvString)"不按车道行驶"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_ACROSS) == PT_ACROSS)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_ACROSS;
                        break;
                    }
                    if (s == (CHvString)"压线"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_OVERLINE) == PT_OVERLINE)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_OVERLINE;
                        break;
                    }
                    if (s == (CHvString)"超速"
                            && (pCarLeftInfo->cCoreResult.ptType & PT_OVERSPEED) == PT_OVERSPEED)
                    {
                        pCarLeftInfo->cCoreResult.ptType = PT_OVERSPEED;
                        break;
                    }
        ...
        BOOL fFirst = TRUE;
        CHvString strMid = "";
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_CONVERSE) == PT_CONVERSE)
        {
            strMid += "逆行;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        }
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_ESTOP) == PT_ESTOP)
        {
            strMid += "非机动车道;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        }
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_RUSH) == PT_RUSH)
        {
            strMid += "闯红灯;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        }
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_ACROSS) == PT_ACROSS)
        {
            strMid += "不按车道行驶;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        }
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_OVERLINE) == PT_OVERLINE)
        {
            strMid += "压线;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        }
        if (!fFound && (pCarLeftInfo->cCoreResult.ptType & PT_OVERSPEED) == PT_OVERSPEED)
        {
            strMid += "超速;";
            if (!m_cModuleParams.cResultSenderParam.nOutputPeccancyType)
                fFound = TRUE;
        ...
        {
            //去掉最后一个分号
            strMid = strMid.Left(strMid.GetLength() - 1);
            strcat(pszConf, "<");
            strcat(pszConf, strMid.GetBuffer());
            strcat(pszConf, ">\n");
        }
    }
    else
    ...
    //压线,车道线从左到右编号从0开始
    if (pCarLeftInfo->cCoreResult.nDetectOverYellowLineEnable && pCarLeftInfo->cCoreResult.coltIsOverYellowLine != COLT_INVALID)
    {
        ...
        else
        {
            ...
            {
                strYellowLine.Format("压线:车道线%d<黄线>\n", pCarLeftInfo->cCoreResult.coltIsOverYellowLine + 1 );
            }
            else
            {
                strYellowLine.Format("压线:车道线%d<实线>\n", pCarLeftInfo->cCoreResult.coltIsOverYellowLine + 1 );
            }
            strcat(pszConf, strYellowLine.GetBuffer());
        }
    }
    //越线,车道线从左到右编号从0开始
    if (pCarLeftInfo->cCoreResult.nDetectCrossLineEnable && pCarLeftInfo->cCoreResult.coltIsCrossLine != COLT_INVALID)
    {
        ...
        else
        {
            ...
            strCrossLine.Format("越线:车道线%d\n", pCarLeftInfo->cCoreResult.coltIsCrossLine + 1);
            strcat(pszConf, strCrossLine.GetBuffer());
        }
    }
    strcat(pszConf, "停止线前:");
    switch (pCarLeftInfo->cCoreResult.tsOnePosScene.lsForward)
    {
    case TLS_UNSURE:
        strcat(pszConf, "直??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsOnePosScene.lsLeft)
    {
    case TLS_UNSURE:
        strcat(pszConf, "左??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsOnePosScene.lsRight)
    {
    case TLS_UNSURE:
        strcat(pszConf, "右??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsOnePosScene.lsTurn)
    {
    case TLS_UNSURE:
        strcat(pszConf, "调??");
        break;
    ...
    strcat(pszConf, "\n停止线后:");
    switch (pCarLeftInfo->cCoreResult.tsTwoPosScene.lsForward)
    {
    case TLS_UNSURE:
        strcat(pszConf, "直??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsTwoPosScene.lsLeft)
    {
    case TLS_UNSURE:
        strcat(pszConf, "左??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsTwoPosScene.lsRight)
    {
    case TLS_UNSURE:
        strcat(pszConf, "右??");
        break;
    ...
    switch (pCarLeftInfo->cCoreResult.tsTwoPosScene.lsTurn)
    {
    case TLS_UNSURE:
        strcat(pszConf, "调??");
        break;
    ...
    CHvString str;
    //左转红灯
    if ( pCarLeftInfo->cCoreResult.lrtRedStart.dwLeftL != 0 )
    {
        REAL_TIME_STRUCT realTime;
        ConvertMsToTime(
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwLeftL,
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwLeftH,
            &realTime
        );
        str.Format(
            "\n左转红灯时间:%02d:%02d:%02d:%03d",
            realTime.wHour, realTime.wMinute, realTime.wSecond,
            (pCarLeftInfo->cCoreResult.lrtRedStart.dwLeftL % 1000)
        );
        strcat(pszConf, str.GetBuffer());
    }
    //直行红灯
    if ( pCarLeftInfo->cCoreResult.lrtRedStart.dwForwardL != 0 )
    {
        REAL_TIME_STRUCT realTime;
        ConvertMsToTime(
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwForwardL,
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwForwardH,
            &realTime
        );
        str.Format(
            "\n直行红灯时间:%02d:%02d:%02d:%03d",
            realTime.wHour, realTime.wMinute, realTime.wSecond,
            (pCarLeftInfo->cCoreResult.lrtRedStart.dwForwardL % 1000)
        );
        strcat(pszConf, str.GetBuffer());
    }
    //右转红灯
    if ( pCarLeftInfo->cCoreResult.lrtRedStart.dwRightL != 0 )
    {
        REAL_TIME_STRUCT realTime;
        ConvertMsToTime(
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwRightL,
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwRightH,
            &realTime
        );
        str.Format(
            "\n右转红灯时间:%02d:%02d:%02d:%03d",
            realTime.wHour, realTime.wMinute, realTime.wSecond,
            (pCarLeftInfo->cCoreResult.lrtRedStart.dwRightL % 1000)
        );
        strcat(pszConf, str.GetBuffer());
    }
    //右转红灯
    if ( pCarLeftInfo->cCoreResult.lrtRedStart.dwTurnL != 0 )
    {
        REAL_TIME_STRUCT realTime;
        ConvertMsToTime(
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwTurnL,
            (DWORD32)pCarLeftInfo->cCoreResult.lrtRedStart.dwTurnH,
            &realTime
        );
        str.Format(
            "\n调头红灯时间:%02d:%02d:%02d:%03d",
            realTime.wHour, realTime.wMinute, realTime.wSecond,
            (pCarLeftInfo->cCoreResult.lrtRedStart.dwTurnL % 1000)
        );
        strcat(pszConf, str.GetBuffer());
    }
    if (pCarLeftInfo->cCoreResult.rcRedLightCount > 0 && pCarLeftInfo->cCoreResult.rcRedLightCount < 20 && pCarLeftInfo->cCoreResult.iAvgY < m_cModuleParams.cResultSenderParam.nRedPosBrightness)
    {
        //添加红灯坐标
        char szMsg[512];
        HV_memset(szMsg, 0, 512);
        sprintf(szMsg, "\nRedLight:");
        char szTmp[32];
        for (int i = 0; i < pCarLeftInfo->cCoreResult.rcRedLightCount; ++i)
        {
            sprintf(szTmp, "(%04d,%04d,%04d,%04d)", pCarLeftInfo->cCoreResult.rcRedLightPos[i].left, pCarLeftInfo->cCoreResult.rcRedLightPos[i].top, pCarLeftInfo->cCoreResult.rcRedLightPos[i].right, pCarLeftInfo->cCoreResult.rcRedLightPos[i].bottom);
            strcat(szMsg, szTmp);
        }
        strcat(pszConf, szMsg);
    }
}

HRESULT CEPolice::SetCaptureSynSignalEnableFlash(bool fIsNight)
{
    HRESULT hr = E_FAIL;
