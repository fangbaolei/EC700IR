...
void Venus_OutputDebugEx(char* szMsg)
{
	return;
	if( g_pFuction != NULL && g_pFuction->pfDebugOutPut != NULL )
	{
...
void Venus_CacheWbInv(void* blockPtr, unsigned int byteCnt)
{
	if( g_pFuction != NULL && g_pFuction->pfCacheWbInv != NULL )
	{
		//Venus_OutputDebugEx("<123XX>Do CacheWbInv.");
		g_pFuction->pfCacheWbInv(blockPtr, byteCnt, 0xFF, 1);
	}
}
...
HRESULT CTrackerImpl::ModifyTracker(TRACKER_CFG_PARAM* pCfgParam)
{
	...
	char szMsg[256];
	sprintf(szMsg, "<modify>detarea:enable:%d,(%d,%d,%d,%d) plate:%d~%d.",
			(int)m_cDetAreaInfo.fEnableDetAreaCtrl,
			...
			m_cDetAreaInfo.nDetectorMinScaleNum,
			m_cDetAreaInfo.nDetectorMaxScaleNum);
...
//		RectifyInfo.fltVAngle = 90 + RectifyInfo.fltHAngle;		// 原垂直矫正有问题 垂直矫正参考水平矫正
	...
	for (int i=0; i<iCharNum; i++)
	{
		...
		case PLATE_DOUBLE_YELLOW:					// 双层牌
			...
			if ((i >= 2) && (i <= 6))
			{
				if (IsMilitaryAlpha(Info.rgbContent[0]) || IsMilitaryChn(Info.rgbContent[0]))  //加入新军牌首字符为字母的判断
				{
					// 首字是军牌汉字
...
)
{
    ...
    if ((imgSnapFrame.iWidth != m_nWidth || imgSnapFrame.iHeight != m_nHeight) || m_fIsModifyParam)
	{
        ...
        m_cDjAppTracker.SetCallBackFunc(NULL);
        m_cDjAppTracker.SetTrackerCallBack(this);
        RTN_HR_IF_FAILED(m_cDjAppTracker.Init(&m_cTrackerCfgParam, m_pParam, 
            m_pHvModel, m_nWidth, m_nHeight, m_pScaleSpeed));
		m_fIsModifyParam = false;			
		//sprintf(g_szDebugInfo + strlen(g_szDebugInfo), "Init OK.\n");
		//sprintf(g_szDebugInfo + strlen(g_szDebugInfo), "imgWH %d %d.\n", imgSnapFrame.iWidth, imgSnapFrame.iHeight);
	}
    ...
    if (s_MaxTime > 20)
    {
        sv::utTrace("@EPCtrlImpl OUT1, MAX JUMP:%d\n", s_MaxTime);
    }
    s_nDelayAll += _nUseTime;
    ++s_nDelayAllCount;
    //dbg

    HRESULT hr = m_cDjAppTracker.Process(pParam, pProcessData, pProcessRespond);
    ...
    {
    	sv::utTrace("@EPCtrlImpl Delay Avg %d\n", s_nDelayAll/s_nDelayAllCount);
    	s_nDelayAllCount = 0;
    	s_nDelayAll = 0;
    }
...
)
{
	...
	char szMsg[256] = {0};
	sprintf( szMsg, "<process><type:%d,w:%d,h:%d>",
			pProcessData->hvImageYuv.nImgType,
			pProcessData->hvImageYuv.iWidth,
			pProcessData->hvImageYuv.iHeight);
	Venus_OutputDebug(szMsg);
...
//	}
	...
	sprintf( szMsg, "<process><detarea:%d,%d,%d,%d>",
				rcTrackArea.left,
				rcTrackArea.top,
				rcTrackArea.right,
				rcTrackArea.bottom);
	...
	sprintf( szMsg, "<process><det,count:%d,time:%d>", nSearchNum, nDetectorTime);
	Venus_OutputDebug(szMsg);
	...
	for (int iRegionIndex = 0; iRegionIndex < nSearchNum; iRegionIndex++)
	{
		...
		for (int i=0; i<MultiROI.dwNumROI; i++)
		{
			...
			RTN_HR_IF_FAILED(RecognizePlate(
					imgPlate, 
					plateNew_Default,
					imgRectified
					));
	...
	sprintf( szMsg, "<process><reg,count:%d,time:%d>", nRegCount, nRegionTime);
	Venus_OutputDebug(szMsg);
	...
	for (int i=0; i<m_cTrackInfo; i++)
	{
		...
		if (pTrack->m_fObservationExists)
		{
			...
			char szPlateTemp[100];
			memset(szPlateTemp, 0, 100);
			plateObserved.GetPlateNameAlpha(szPlateTemp);
			sprintf(szMsg,"<process second>plate:%s.", (char*)szPlateTemp);
			Venus_OutputDebug(szMsg);
		}
	...
	sprintf( szMsg, "<second,count:%d,time:%d>", iDoCount, nSecondRecog);
	Venus_OutputDebug(szMsg);
  ...
  //					HV_Trace("\n<Lost>x: %d,%d y:%d,%d \n", nXmove, pTrack->m_nMaxXPos, nYMove, pTrack->m_nMaxYPos);
	...
	if (m_nVideoDetMode == USE_VIDEODET)
	{
...
)
{
	...
	if (pTrack->m_fObservationExists)
	{
		...
		CRect rcVideoDet;
		RTN_HR_IF_FAILED(MapParamRect(rcVideoDet, rcMax, m_rcVideoDetArea));
		rectIntersect.IntersectRect(&plateObserved.rcPos, &rcVideoDet);
...
//	pCurResult->nFirstFrameTime = ObjTrackInfo.m_pFirstRefImage->GetRefTime();
    ...
    if( ObjTrackInfo.m_fHasTrigger && ObjTrackInfo.m_dwTriggerTimeMs != 0 )
	{
		pCurResult->nCarArriveTime = ObjTrackInfo.m_dwTriggerTimeMs;
	}
	else
	{
		pCurResult->nCarArriveTime = ObjTrackInfo.m_nCarArriveRealTime;
	}
...
//	}
	...
	for (int i = 0; i < m_cObjTrackInfo; i++)
	{
		...
		if (m_rgObjTrackInfo[i].m_objTrackState == OBJ_TRACK_MOVING &&
			...
			!fPlateTracking
			&& m_rgObjTrackInfo[i].m_iObjInfoCount >= 5)
		{
...
HRESULT CTrackerImpl::SetFirstLightType(LIGHT_TYPE nLightType)
{
	...
	m_dwLastCarLeftTime = GetSystemTick();
	// zhaopy
	m_cDjAppTracker.SetFirstLightType(nLightType);
	return S_OK;
}
