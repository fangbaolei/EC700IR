...
{
	const char* pValueBegin = NULL;
	const char* pValueEnd = NULL;

	const char* pIdBegin = strstr(pCmd , pId);
	if (NULL != pIdBegin)
	{
		...
		{
			int nValueLen = (int)((pValueEnd - 1) - (pValueBegin +1)+1);
...
HRESULT ReadRecordCacheInfo(HVAPI_HANDLE_CONTEXT_EX* pHandle ,LPCSTR pRecordConCmd , INT& nRecordConCmdLen)
{
	...
	sprintf(szTemp , "DownloadRecord,"
		...
		,pHandle->iRealTimeRecordSendIllegalVideo
		,pPackResumeCache->nDataOffset
		,pPackResumeCache->nInfoOffset
		);

	if (nRecordConCmdLen > (int)strlen(szTemp) + 1)
	{
		strcpy((char*)pRecordConCmd , szTemp);
		nRecordConCmdLen = (INT)strlen(pRecordConCmd);
...
HRESULT GetAutoLinkConnCmd(HVAPI_HANDLE_CONTEXT_EX* pHandle , BOOL fSafeSaverEnable ,LPCSTR pAutoLinkConnCmd ,INT& nAutoLinkConnCmdLen)
{
	...
	{
		if ((INT)strlen((char*)pHandle->szRecordConnCmd) < nAutoLinkConnCmdLen)
		{
			strcpy((char*)pAutoLinkConnCmd , (char*)pHandle->szRecordConnCmd);
			nAutoLinkConnCmdLen = (INT)strlen((char*)pHandle->szRecordConnCmd);
		}
...
HRESULT GetConnectionRequest(SOCKET hSocket ,LPSTR lpszDevSN , BOOL& fSafeSaverEnable , int* piProtocolVersion )
{
	HRESULT hr = E_FAIL;
	int iRecvBufLen = 0;
				...
				if ( 0 == strcmp("HvCmd", pRootElement->Value()) )
				{
					if(0 == strcmp("ConnectionRequest" ,  pRootElement->FirstChildElement("CmdName")->GetText()))
					{
						...
						if ( pCmdArgElement != NULL)
						{
							...
							{
								if( NULL != piProtocolVersion )
								{
									GetParamIntFromXml(pCmdArgElement , "ProtocolVersion" , piProtocolVersion ) ;
								}
								hr = S_OK;
							}
							else
...
static DWORD WINAPI ProcessRecordNewLinkThreadFunc(LPVOID lpParameter)
{
	...
	if ( S_OK != GetConnectionRequest(hNewSocket , rgDevSN , fSafeSaverEnable , &iProtocolVersion ) || strstr(rgDevSN, "null"))
	{
	...
	else
	{
		...
		int len = sizeof(sa);
		if(!getpeername(hNewSocket, (struct sockaddr *)&sa, &len)) 
		{
			pADC->dwConnPort = ntohs(sa.sin_port);
			sprintf(pADC->handle.szIP ,"%s", inet_ntoa(sa.sin_addr));
		}
	}
	...
	pHHC->emProtocolVersion = iProtocolVersion == PROTOCOL_MERCURY?PROTOCOL_VERSION_MERCURY:PROTOCOL_VERSION_2;
	...
	if (NULL != pADC)
	{
		...
		if (DEV_STATUS_SETCONN == pADC->eStatus && 
			...
			CONN_STATUS_CONNOVERTIME == pHHC->dwRecordConnStatus ))
		{
...
HRESULT SetNewLink(LPVOID lpParameter ,SOCKET hNewSocket )
{
	...
	*/
	...
	if (LISTEN_TYPE_RECORD == pServer->nMonitorType /* && NULL != phFreeThread */)
	{
		...
		//	return E_FAIL;
		//}
		HANDLE	hNewThread = CreateThread(NULL, 0, ProcessRecordNewLinkThreadFunc, (LPVOID)hNewSocket, 0, NULL);
...
HRESULT GetAutoLinkCount(INT nType , INT & nLinkCount)
{
	...
	for (int i = 0 ; i<g_pMonitor->nMaxMonitorCount ; ++i)
	{
		...
		if (!fFreeDev)
		{
			if((pHHC->AutoLinkSetCallBackTime != 0) 
				&& pHHC->AutoLinkSetCallBackTime + 5 < time(NULL)
				&& pHHC->AutiLinkOverTime == 0)
			{
				pHHC->dwRecordConnStatus = CONN_STATUS_DISCONN;
			...
			if(pHHC->AutiLinkOverTime != 0
				&&(pHHC->dwRecordConnStatus == CONN_STATUS_DISCONN || pHHC->dwRecordConnStatus == CONN_STATUS_RECONN)
...
HV_API int CDECL HVAPI_GetDeviceList(LPSTR szDevList , INT nDevListLen , LPCSTR szApiVer)
{
	...
	for (int i = 0; i <g_pMonitor->nMaxMonitorCount ; ++i)
	{
		...
		if (pHHC->fAutoLink && !fDevFree)
		{
			int nStrlen = (int)(strlen(szDevList) + strlen(pHHC->szDevSN) + 2);
			if ((INT)(strlen(szDevList) + strlen(pHHC->szDevSN) + 2) <= nDevListLen)
			{
...
HRESULT UpdatePackCache(HVAPI_HANDLE_CONTEXT_EX* pHandle, PACK_TYPE eType , char* pBuffer , int nBufferLen )
{
	...
	if (PACK_TYPE_HEADER == eType)
	{
		...
		else if(0 == pPackResumeCache->nInforLen && 0 == pPackResumeCache->nDataLen)
		{
			if (nBufferLen == sizeof(pPackResumeCache->header))
			{
				...
				if (dw32MaxInfoLen > (DWORD32)(pPackResumeCache->nMaxInforLen) && dw32MaxInfoLen< MAX_INFOR_LEN)
				{
					if (NULL != pPackResumeCache->pInfor)
					{
						delete pPackResumeCache->pInfor;
				...
				if (dw32MaxDataLen > (DWORD32)(pPackResumeCache->nMaxDataLen) && dw32MaxDataLen < MAX_DATA_LEN)
				{
					if (NULL != pPackResumeCache->pData)
					{
	...
	if (PACK_TYPE_INFO == eType)
	{
		...
		else
		{
			...
			if ((DWORD32)(nBufferLen + pPackResumeCache->nInforLen) <= (DWORD32)(pPackResumeCache->header.dwInfoLen) && 
				pPackResumeCache->header.dwInfoLen <= (DWORD32)(pPackResumeCache->nMaxInforLen))
			{
	...
	if (PACK_TYPE_DATA == eType)
	{
		...
		else
		{
			...
			if ((DWORD32)(pPackResumeCache->nDataLen + nBufferLen) <= (DWORD32)(pPackResumeCache->header.dwDataLen) &&
				pPackResumeCache->header.dwDataLen <= (DWORD32)(pPackResumeCache->nMaxDataLen))
			{
...
HV_API HRESULT CDECL HVAPI_GetDeviceInfoAutoLink(LPCSTR szDevName, LPCSTR szApiVer, LPSTR strIP, INT strLen, INT& dwPORT)
{
	...
	if ( 0 == strcmp(HVAPI_API_VERSION_EX, szApiVer) )
	{
		...
		{		
				...
				for (int i = 0 ; i<g_pMonitor->nMaxMonitorCount ; ++i)
				{
					pADC = &g_pMonitor->prgAutoLinkDevList[i];
					if (!pADC->fFree)
					{
						if (0 == strcmp(pADC->handle.szDevSN , szDevName))
						{
							strcpy(strIP , pADC->handle.szIP);
							dwPORT = pADC->dwConnPort;
							LeaveCriticalSection(& g_csDevList);
							return S_OK;
						}	
					}
