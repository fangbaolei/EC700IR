...
CUpdateDlg::CUpdateDlg(HVAPI_HANDLE_EX* hHandle, CWnd* pParent /* = NULL */)
...
#define WM_USERDEFMSG_PUP   (WM_USER + 107) 
...
BOOL CUpdateDlg::OnInitDialog()
{
	...
	if(iWorkMode == 2)
	{
		...
		int iPageIndex = 0;
		int iRetLen = 0;
		if(HVAPI_ExecCmdEx(*m_HvDevice, "GetControllPannelUpdatingStatus", 
			szRetBuf, sizeof(szRetBuf), &iRetLen) == S_OK)
		{
#ifdef OLD_XML_FORMAT
			if(HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelUpdatingStatus", "Status", szValue) == S_OK)
#else
			if(HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelUpdatingStatus", "Status", szValue) == S_OK)
#endif
...
UINT CUpdateDlg::ProcessUpDateThread(LPVOID pUserData)
{
	...
	while(pDlg->m_fIsRunningUpdata)
	{
		...
		case 1:
			{
				...
				char szRet[256] = {0};
				int iRetLen = 0;
				if(HVAPI_ExecCmdEx(*pDlg->m_HvDevice, "SetControllPannelStatus,WorkMode[2],WorkStatus[0]", szRet, sizeof(szRet), &iRetLen) != S_OK)
				{
		...
		//		if(FAILED(HvParseXmlCmdRespRetcode(szRet, szRetcode)))
				if(FAILED(HVAPIUTILS_GetExeCmdRetInfoEx(false, szRet, "SetControllPannelStatus", "RetCode", szRetcode)))
#else
				if(FAILED(HVAPIUTILS_GetExeCmdRetInfoEx(true, szRet, "SetControllPannelStatus", "RetCode", szRetcode)))
#endif
		...
		case 2:
			...
			if(HVAPI_SendControllPannelUpdateFileEx(*pDlg->m_HvDevice, (BYTE*)pDlg->m_rgbUpdateFileBuffer, UPDATE_FILE_SIZE) != S_OK)
			{
				char szRet[256] = {0};
				int iRetLen = 0;
				if(HVAPI_ExecCmdEx(*pDlg->m_HvDevice, "SetControllPannelStatus,WorkMode[1],WorkStatus[0]", szRet, sizeof(szRet), &iRetLen) != S_OK)
				{
		...
		case 3:
			{
				...
				int iPageIndex = 0;
				int iRetLen = 0;
				if(HVAPI_ExecCmdEx(*pDlg->m_HvDevice, "GetControllPannelUpdatingStatus", 
					szRetBuf, sizeof(szRetBuf), &iRetLen) != S_OK)
				{
					Sleep(100);
					continue;
				...
				memset(szValue, 0, 20);
#ifdef OLD_XML_FORMAT
				if(HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelUpdatingStatus", "PageIndex", szValue) != S_OK)
#else
				if(HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelUpdatingStatus", "PageIndex", szValue) != S_OK)
#endif
				{
					Sleep(100);
					continue;
		...
		case 4:
			{
				...
				int iCRCValue = -1;
				int iRetLen = 0;
				if(HVAPI_ExecCmdEx(*pDlg->m_HvDevice, "GetControllPannelCRCValue", 
					szRetBuf, sizeof(szRetBuf), &iRetLen) != S_OK)
				{
					Sleep(100);
					continue;
...
#ifdef OLD_XML_FORMAT
				if(HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelCRCValue", "Value", szValue) != S_OK)
#else
				if(HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelCRCValue", "Value", szValue) != S_OK)
#endif
				{
					Sleep(100);
					continue;
		...
		case 1:
			{
				...
				{
#ifdef OLD_XML_FORMAT
					HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelCRCValue", "Value", szValue);
#else
					HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelCRCValue", "Value", szValue);
#endif
...
void CUpdateDlg::GetControllPannelStatus(int& iWorkMode, int& iWorkStatus)
{
	...
	if(HVAPI_ExecCmdEx(*m_HvDevice, "GetControllPannelStatus", 
		szRetBuf, sizeof(szRetBuf), &iRetLen) != S_OK)
	{
	...
	{
#ifdef OLD_XML_FORMAT
		if(HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelStatus", "WorkMode", szWorkMode) != S_OK)
#else
		if(HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelStatus", "WorkMode", szWorkMode) != S_OK)
#endif
		{
			iWorkMode = -1;
		}
#ifdef OLD_XML_FORMAT
		if(HVAPIUTILS_GetExeCmdRetInfoEx(FALSE, szRetBuf, "GetControllPannelStatus", "WorkStatus", szWorkStatus) != S_OK)
#else
		if(HVAPIUTILS_GetExeCmdRetInfoEx(TRUE, szRetBuf, "GetControllPannelStatus", "WorkStatus", szWorkStatus) != S_OK)
#endif
		{
			iWorkStatus = -1;
		}
