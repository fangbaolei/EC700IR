...
int GetSTimeByDWORD64(STime &stime , DWORD64 dw64Time)
{
	...
	stime.nMs = dw64Time%1000;

	struct tm *ts= localtime(&sec);

	stime.nYear = ts->tm_year + 1900;
	stime.nMonth = ts->tm_mon + 1;
	stime.nDay = ts->tm_mday;
	stime.nHour = ts->tm_hour;
	stime.nMin = ts->tm_min;
	stime.nSec = ts->tm_sec;
...
//	((CEdit*)GetDlgItem(IDC_DIR))->SetWindowText("E:\\");
	...
	((CEdit*)GetDlgItem(IDC_DIR))->SetWindowText("D:\\");
	OnBnClickedButton1();
		...
		if (S_OK == HVAPI_GetDeviceList(pDevListBuf , nDevListBufLen , NULL))
		{
			...
			while(pszTemp = strstr(pszSN ,";"))
			{
				*pszTemp = '\0';
				...
				pDlg->ShowClientInfor(pszSN,szRecordConStatus,nReConnectCount );
				pszSN = pszTemp+1;
			}
		}
	}
...
HRESULT CAutoLink_demoDlg::ShowClientInfor( char* pSN, char* pStatus,int nReConnectCount )
{
	if (m_fExit || NULL == pSN)
	{
		return S_OK;
	...
	if (0 == strcmp(pStatus ,DEV_STATUS_DONE))
	{
		CString strDevSn = pSN;
		DEV_CONFIG devConfig;
		ReadConfig(strDevSn.GetBuffer() ,&devConfig );
		if (0 == strcmp(devConfig.szEndTime  ,"0"))
		{
			DisConnectDevice(pSN, DEV_STATUS_CONN);
		}
	...
	for (int index = 0; index < iNetDiskCount ; ++index)
	{
		if (m_pIsUsrDiscnn[index] == 0)
		{
			CString strDevSn = m_devListCtrl.GetItemText(index , 0);
			CString strConnStatus = m_devListCtrl.GetItemText(index , 1);
			AutoLinkToDevice(strDevSn, strConnStatus);
		}
	}
	...
	for (int i = 0; i<iNetDiskCount ; i++)
	{
		...
		{
			m_devListCtrl.SetItemText(i , 1 , pStatus);
	...
	m_devListCtrl.SetItem(iItemRow ,1 , LVIF_TEXT , pStatus , 0 , 0 ,0 ,0 );
	m_devListCtrl.SetItemText(i , 2 , strReConnectCount);
					...
					0 == strcmp(strConnStatus , DEV_STATUS_RECONN ))
		{
			...
			while (pos)
			{
				...
				if (NULL != pDevHandle)
				{
					if (0 == strcmp(pDevHandle->szDevSN ,strDevSn ))
					{
						HVAPI_CloseEx(pDevHandle->handle);
...
void CAutoLink_demoDlg::AutoLinkToDevice(CString strDevSn, CString strConnStatus)
{
	if (0 == strcmp(strConnStatus, DEV_STATUS_UNKNOW) ||
		0 == strcmp(strConnStatus , DEV_STATUS_DISCONN))
	{
...
void CAutoLink_demoDlg::DisConnectDevice(CString strDevSn, CString strConnStatus)
{
	 if (0 == strcmp(strConnStatus , DEV_STATUS_CONN ))
	 {
		 DEV_HANDLE * pDevHandle = NULL;
		 POSITION pos = m_connDevList.GetHeadPosition();
		while (pos)
