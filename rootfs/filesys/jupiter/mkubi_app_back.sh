...
#必要的文件
APP_FILES="backup_sys_app/deamon backup_sys_app/log backup_sys_app/command backup_sys_app/UpgradeApp backup_sys_app/command.sh backup_sys_app/switchsys"
XML_FILES="backup_sys_app/deamon.xml"
INIT_FILE="backup_sys_app/my_run.sh"

APP_FILES1="../backup_sys_app/deamon ../backup_sys_app/log ../backup_sys_app/command ../backup_sys_app/UpgradeApp ../backup_sys_app/command.sh ../backup_sys_app/switchsys"
...
# 复制采集图像程序
cp -a $OUTPUT_DIR/backup_sys_app/video_test/* $VIDEO_APP_DIR

# 复制启动脚本
cp -a $INIT_FILE $APP_DIR
...
#step 2:生成ubi镜像
chmod +x $INPUT_DIR/bin/mkfs.ubifs
chmod +x $INPUT_DIR/bin/ubinize

#-c 471 was the result of calculation
echo "2. generating ubi file"

./$INPUT_DIR/bin/mkfs.ubifs -r $TMP_DIR -m 2048 -e 126976 -c 416 -o $TMPFILE
./$INPUT_DIR/bin/ubinize -o $OUTPUT_DIR/$OUTPUT_IMAGE -m 2048 -p 128KiB -s 2048 -O 2048 $CFG_FILE

# 工厂烧写(uboot烧写)专用
cp -f $TMPFILE $OUTPUT_DIR/$FACTORY_IMAGE
