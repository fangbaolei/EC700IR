 ...
 *         设备相关API。
 * @note
 *         1、目前包括的模块有：eeprom、flash、cpld、fpga、加密认证、看门狗、
 *         LED灯、蜂鸣器、温度、获取版本号(CPLD/uboot/sn)
 ...
 *
 * @note
 *       1、timeout_ms如果为0，则超时时间为默认值：20s(即20000ms)，非0则为所设置的时间
 *       2、超时时间范围：0 < time <= 60s(60000ms)
 */
 ...
 *
 * @note
 *         1、dev_id: DEVID_BUZZER
 *         2、水星平台无此功能
 *
 */
int drv_buzzer_on(DRV_DEVICE_TYPE dev_id);
 ...
 *
 */
int drv_buzzer_off(DRV_DEVICE_TYPE dev_id);
 ...
 *
 * @note
 *        1、根据不同ID号复位不同的设备(如DM368和USB设备)
 *        2、目前支持的设备ID有：DEVID_SPI_FPGA、DEVID_PHY、DEVID_USB、DEVID_DM368、DEVID_DM6467
 *        3、水星平台<b>必须</b>在加载完FPGA 10ms后进行一次FPGA复位，作用：初始化FPGA所有寄存器。
 */
