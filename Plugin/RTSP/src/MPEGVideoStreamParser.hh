...
protected:
  ...
  /*
  void saveAFrame(){
 	const unsigned char *ptr = getNextToParse();
	unsigned TotNumValidBytes = totNumValidBytes();
	if((fTo + (TotNumValidBytes - curOffset())) > fLimit){
		fNumTruncatedBytes += TotNumValidBytes;
		return;
	}
	memcpy(fTo, ptr, TotNumValidBytes - curOffset());
	fTo += TotNumValidBytes - curOffset();
  }
  */

  void saveANALU(int NALUSize){
  	const unsigned char *ptr = getSavedParse();
	if((fTo + NALUSize) > fLimit){
		fNumTruncatedBytes += NALUSize;
		return;
	}
	memcpy(fTo, ptr, NALUSize);
	fTo += NALUSize;
  }

  int getCurFrameSize(){
  	return totNumValidBytes() - curOffset();
  }
