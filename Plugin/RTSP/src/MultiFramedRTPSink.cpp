...
void MultiFramedRTPSink::delayLoop(unsigned long usec)
{
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
	if (fNumFramesUsedSoFar > 0) {
		...
		//delay for UDP send
		unsigned delayMax = 25000;  //30 ms
		unsigned delayTotal = 0;
		struct timeval timeNow;
		gettimeofday(&timeNow, NULL);

		if(fSource->getValidDataBytes() != fCurrentFrameSize){
			fCurrentFrameSize = fSource->getValidDataBytes();
			if((fCurrentFrameSize / 1500) != 0)
				fDelayUnit = delayMax / (fCurrentFrameSize / 1500);
			else
				fDelayUnit = 0;
			fOriginTimeStamp = timeNow.tv_sec * 1000000 + timeNow.tv_usec;
			fDelayTimeStamp = fOriginTimeStamp + fDelayUnit;
		}else{
			int uSecondDiff = 0;
			uint64_t nowTimeStamp = timeNow.tv_sec * 1000000 + timeNow.tv_usec;
			
			uSecondDiff = fDelayTimeStamp - nowTimeStamp;
			if(uSecondDiff > delayMax){
				uSecondDiff = 0;
			}
			if((uSecondDiff > 0) && ((fOriginTimeStamp + delayMax) > nowTimeStamp)){
				delayTotal = uSecondDiff;
			}

			fDelayTimeStamp += fDelayUnit;
		}
		// Send the packet:
#ifdef TEST_LOSS
		if ((our_random()%10) != 0) // simulate 10% packet loss #####
#endif
		if (!fRTPInterface.sendPacket(fOutBuf->packet(), fOutBuf->curPacketSize(), delayTotal)) {
