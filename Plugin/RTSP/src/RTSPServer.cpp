...
Boolean RTSPServer::setUpConnectionStatus(Port connPort){
	fConnStatusSocket = setUpOurSocket(envir(), connPort);
	if(fConnStatusSocket >= 0){
		envir().taskScheduler().turnOnBackgroundReadHandling(fConnStatusSocket,
															 (TaskScheduler::BackgroundHandlerProc *)&incomingConnStatusHandler, this);
		return True;
	}
...
RTSPServer::RTSPServer(UsageEnvironment& env,
	...
	fConnStatusClientSocket = -1;  
	fStatisticsInterval = 5;    		//5 seconds
	fReduceAdjustThreshold = 100; 	//reduce bitrate when more than 100 packets loss
	fNextAscendInterval = 60;		//1 minute
	memset(fLostPacketsStatistics, 0, sizeof(struct lostPacketsStatistics) * RTSP_STREAM_MAX_NUM);
...
void RTSPServer::incomingConnectionHandler(int serverSocket) {
	...
	increaseSendBufferTo(envir(), clientSocket, 512*1024); //default 50*1024
...
void RTSPServer::incomingConnStatusHandler(void *instance, int ){
	RTSPServer *server = (RTSPServer *)instance;
	server->incomingConnStatusHandler1();
}

void RTSPServer::incomingConnStatusHandler1(){
	struct sockaddr_in clientAddr;
	struct timeval timeout = {0, 40000};
	SOCKLEN_T clientAddrLen = sizeof clientAddr;

	int serverSocket = fConnStatusSocket;
	int clientSocket = accept(serverSocket, (struct sockaddr*)&clientAddr, (socklen_t *)&clientAddrLen);
	if (clientSocket < 0) {
		int err = envir().getErrno();
		if (err != EWOULDBLOCK) {
	...
	// Create a new object for handling this RTSP connection:
	//(void)createNewClientConnection(clientSocket, clientAddr);
	// envir().taskScheduler().setBackgroundHandling(clientSocket, 
	// 												SOCKET_READABLE,
	// 						(TaskScheduler::BackgroundHandlerProc*)&incomingGetRTSPStatusHandler, this);
	if(-1 != fConnStatusClientSocket){
		::closeSocket(fConnStatusClientSocket);	  
	}

	fConnStatusClientSocket = clientSocket;
}


Boolean RTSPServer::getConnectionStatusRequestHandler(int clientSocket, ServerMediaSubsession *subsession, E_H264_STATUS eStatusCmd){
	if(subsession == NULL)
		return false;
	int connections = 0;
	unsigned int offset = 0;
	unsigned int sendBufLen = 2048;
	unsigned int sendBufOffset = 1;
	char sendBuf[2048] = {0};
	H264_STATUS_HEADER cStatusHeader;
	memset(&cStatusHeader, 0, sizeof(H264_STATUS_HEADER));
	sendBuf[0] = '_';//start char
	if(subsession->getLastStreamToken() != NULL){
		connections = ((StreamState*)(subsession->getLastStreamToken()))->referenceCount();
		HashTable *destTable = subsession->getDestinationsHashTable();
		if(destTable != NULL && destTable->numEntries() > 0){
			Destinations* destinations;
			struct sockaddr_in addr_in;
			int addrLen = sizeof(struct sockaddr);
			char const* key; // dummy
			HashTable::Iterator* iter = HashTable::Iterator::create(*destTable);
			while ((destinations = (Destinations *)(iter->next(key))) != NULL) {
				char destStr[64] = {0};
				snprintf(destStr, sizeof(destStr)-1, "%s,%d_",
						 inet_ntoa(destinations->addr),
						 ntohs(destinations->rtpPort.num()));
				strncat(sendBuf, destStr, sendBufLen);
				sendBufOffset += strlen(destStr);
				sendBufLen -= sendBufOffset;
				offset++;
				if(sendBufOffset >= sizeof(destStr))
					break;
			}
			delete iter;
		}
	...
	if(connections == offset){
		...
		if(sendBufLen == 0)
			sendBufLen = 1;
		cStatusHeader.eH264StatusCmd = eStatusCmd;
		cStatusHeader.iDataLen = sendBufLen;
		if(send(clientSocket, &cStatusHeader, sizeof(H264_STATUS_HEADER), 0) > 0){
			int sendLen = send(clientSocket, sendBuf, sendBufLen, 0);
			if(sendLen != sendBufLen || sendLen < 0){
				fprintf(stderr, "[RTSP %s-%d]: Send connection status info error,%d:%s  %d %d\n", 
						__FILE__, __LINE__, errno, strerror(errno), sendLen, sendBufLen);
				return False;
			}
		}else{
			fprintf(stderr, "[RTSP %s-%d]: Send connection status info length error\n", __FILE__, __LINE__);
			return False;
		}
	}else{
		fprintf(stderr, "[RTSP %s-%d]: Get connection count error, %d-%d\n", __FILE__, __LINE__, connections, offset);
		return False;
	}
...
void RTSPServer::incomingGetRTSPStatusHandler1(E_H264_STATUS command){
	ServerMediaSession *session = NULL;
	ServerMediaSubsession *subsession = NULL;
	Boolean ret = False;
	if(-1 == fConnStatusClientSocket)
		return;
	int handledSocketNum = fConnStatusClientSocket;

	switch(command){
	case H264_STATUS_FIRST_STREAM_CONN:
		session = lookupServerMediaSession(fStreamName[0]);
		break;
	case H264_STATUS_SECOND_STREAM_CONN:
		session = lookupServerMediaSession(fStreamName[1]);
		break;
	default:
		break;
	}
	...
	if(session != NULL){
		ServerMediaSubsessionIterator subIter(*session);
		subsession = subIter.next();
	}else{
		return;
	}
	if(subsession == NULL)
	...
	if(!(ret = getConnectionStatusRequestHandler(handledSocketNum, subsession, command)))
		fprintf(stderr, "[RTSP: %s-%d] Get rtsp second stream status error!\n", __FILE__, __LINE__);
	
	if(ret == False){
		// envir().taskScheduler().disableBackgroundHandling(handledSocketNum);
		::closeSocket(handledSocketNum);
		fConnStatusClientSocket = -1;
		fprintf(stderr, "[RTSP: %s-%d] Get rtsp status error\n", __FILE__, __LINE__);
		return;
	}
}

void RTSPServer::reduceBitrateHandler(unsigned int streamIndex)
{
	H264_STATUS_HEADER cStatusHeader;
	...
	memset(&cStatusHeader, 0, sizeof(H264_STATUS_HEADER));
	connectCount = fClientConnections->numEntries();

	if(streamIndex == 0)
		cStatusHeader.eH264StatusCmd = H264_STATUS_FIRST_STREAM_REDUCE;
	else
		cStatusHeader.eH264StatusCmd = H264_STATUS_SECOND_STREAM_REDUCE;
	cStatusHeader.iDataLen = sizeof(unsigned int);

	if(send(fConnStatusClientSocket, &cStatusHeader, sizeof(H264_STATUS_HEADER), 0) > 0){
		if(send(fConnStatusClientSocket, &connectCount, sizeof(unsigned int), 0) < 0){
			fprintf(stderr, "[RTSP: %s-%d] REDUCE stream bitrate status error!\n", __FILE__, __LINE__);
		}
	}
}

void RTSPServer::ascendBitrateHandler(unsigned int streamIndex)
{
	H264_STATUS_HEADER cStatusHeader;
	unsigned int connectCount = 0;
	if(-1 == fConnStatusClientSocket)
	...
	if(streamIndex == 0)
		cStatusHeader.eH264StatusCmd = H264_STATUS_FIRST_STREAM_ASCEND;
	else
		cStatusHeader.eH264StatusCmd = H264_STATUS_SECOND_STREAM_ASCEND;
	cStatusHeader.iDataLen = sizeof(unsigned int);

	if(send(fConnStatusClientSocket, &cStatusHeader, sizeof(H264_STATUS_HEADER), 0) > 0){
		if(send(fConnStatusClientSocket, &connectCount, sizeof(unsigned int), 0) < 0){
			fprintf(stderr, "[RTSP: %s-%d] ASCEND stream bitrate status error!\n", __FILE__, __LINE__);
		}
	}
}

void RTSPServer::setStreamName(unsigned int index, const char *streamName, unsigned int nameLen)
{
	if(index < 0 || index >= RTSP_STREAM_MAX_NUM || streamName == NULL)
		return;

	strncpy(fStreamName[index], streamName, sizeof(fStreamName[0]) - 1);
}

void RTSPServer::getStreamName(unsigned int index, char *streamName, unsigned int *nameLen)
{
	if(index < 0 || index >= RTSP_STREAM_MAX_NUM || streamName == NULL)
		return;

	if(*nameLen > (sizeof(fStreamName[0]) - 1))
		*nameLen = sizeof(fStreamName[0]) - 1;

	strncpy(streamName, fStreamName[index], *nameLen);
}


void RTSPServer::rtcpRRHandler(RTSPClientSession* curClientSession)
{
	RTSPServer::RTSPClientSession* clientSession = NULL;
	int streamIndex = 0;
	unsigned int tmpTotalPacketLost = 0;
	E_H264_STATUS connStatusCommand;
	struct timeval timeNow;

	//不用自动控制调整
	if(0 == getAutoControlBitrate())
		return;

	for(int i = 0; i < RTSP_STREAM_MAX_NUM; i++){
		if(0 == strncmp(curClientSession->getServerMediaSession()->streamName(), 
						fStreamName[i], 63)){
			streamIndex = i;
			break;
		}
	}
	if(RTSP_STREAM_MAX_NUM == streamIndex)	//never come to here!!!
		return;

    gettimeofday(&timeNow, NULL);
	//5 seconds check one time
	if((timeNow.tv_sec - fLostPacketsStatistics[streamIndex].lastStatisticsTime) < fStatisticsInterval)
		return;

	if(curClientSession->fNumStreamStates <= 0)
		return;

	RTPTransmissionStatsDB &tsdb = ((StreamState *)curClientSession->fStreamStates[0].streamToken)->rtpSink()->transmissionStatsDB();
	RTPTransmissionStatsDB::Iterator iter(tsdb);
	RTPTransmissionStats *ts = iter.next();
	while(ts){
		tmpTotalPacketLost += ts->totNumPacketsLost();
		ts = iter.next();
	}

	tmpTotalPacketLost = tmpTotalPacketLost < 0 ? 0 : tmpTotalPacketLost;

	if(tmpTotalPacketLost < fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost)
		fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost = tmpTotalPacketLost;

	fLostPacketsStatistics[streamIndex].lastStatisticsTime = timeNow.tv_sec;

	if((tmpTotalPacketLost - fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost) > fReduceAdjustThreshold){
		fprintf(stdout, "*****************loss packet:%d tmpTotal:%d lastTotal:%d\n", 
				tmpTotalPacketLost - fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost, 
				tmpTotalPacketLost, fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost);
		fLostPacketsStatistics[streamIndex].lastReduceTime = timeNow.tv_sec;
		fLostPacketsStatistics[streamIndex].nextAscendTime = timeNow.tv_sec + (fNextAscendInterval * 3);//1 hour
		reduceBitrateHandler(streamIndex);
	}

	if(((tmpTotalPacketLost - fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost) < 20) &&
	   (0 != fLostPacketsStatistics[streamIndex].nextAscendTime) &&
	   (timeNow.tv_sec > fLostPacketsStatistics[streamIndex].nextAscendTime)){

		fLostPacketsStatistics[streamIndex].nextAscendTime = timeNow.tv_sec + fNextAscendInterval;		//reset
		ascendBitrateHandler(streamIndex);		   
	}

	fLostPacketsStatistics[streamIndex].lastTotalNumPacketsLost = tmpTotalPacketLost;


//	incomingGetRTSPStatusHandler1(H264_STATUS_FIRST_STREAM_CONN);
//	incomingGetRTSPStatusHandler1(H264_STATUS_SECOND_STREAM_CONN);
...
::authenticationOK(char const* cmdName, char const* urlSuffix, char const* fullRequestStr) {
	...
	if(-1 == fOurServer.fConnStatusClientSocket)
		return false;
	...
	do {
		...
		cStatusHeader.eH264StatusCmd = H264_STATUS_AUTHENTICATE;
		cStatusHeader.iDataLen = strlen(username);

		if(send(fOurServer.fConnStatusClientSocket, &cStatusHeader, sizeof(H264_STATUS_HEADER), 0) < 0){
			fprintf(stderr, "[RTSP: %s-%d] user authenticate error!\n", __FILE__, __LINE__);
			return false;
		}
		if(send(fOurServer.fConnStatusClientSocket, username, strlen(username), 0) < 0){
			fprintf(stderr, "[RTSP: %s-%d] user authenticate error!\n", __FILE__, __LINE__);
			return false;
		}
		
		if(recv(fOurServer.fConnStatusClientSocket, password, sizeof(password), 0) <= 0){
			fprintf(stderr, "[RTSP: %s-%d] user authenticate error!\n", __FILE__, __LINE__);
			return false;
		}
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
	...
	do {
		...
		if(streamingMode == RTP_TCP){
			clientRTPPortNum = ntohs(ourClientConnection->fClientAddr.sin_port);
			clientRTCPPortNum = ntohs(ourClientConnection->fClientAddr.sin_port);
		}
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection* ourClientConnection,
	...
	//note RTSP connection change
	if ((strcmp(cmdName, "TEARDOWN") == 0) || (strcmp(cmdName, "PLAY") == 0)) {
		fOurServer.incomingGetRTSPStatusHandler1(H264_STATUS_FIRST_STREAM_CONN);
		fOurServer.incomingGetRTSPStatusHandler1(H264_STATUS_SECOND_STREAM_CONN);
	}
}
...
::noteClientLiveness(RTSPClientSession* clientSession) {
	...
	clientSession->fOurServer.rtcpRRHandler(clientSession);
}
