...
portNumBits OnDemandServerMediaSubsession::fLastInitialPortNum = 0;
...
::OnDemandServerMediaSubsession(UsageEnvironment& env,
  ...
  fCNAME[sizeof fCNAME-1] = '\0'; // just in case
	...
	if(0 == fLastInitialPortNum){
		fLastInitialPortNum = initialPortNum;
	}else{
		fInitialPortNum = fLastInitialPortNum + 2;
		fLastInitialPortNum = fInitialPortNum;
	}
}
...
OnDemandServerMediaSubsession::sdpLines() {
  if (fSDPLines == NULL) {
    ...
    unsigned estBitrate;
    unsigned isNewSource = 0;
    FramedSource *inputSource = NULL;
    if(fLastStreamToken != NULL && 
		    (((StreamState*)fLastStreamToken)->referenceCount() > 0)){
    	inputSource = ((StreamState*)fLastStreamToken)->mediaSource();
	isNewSource = 0;
    } else {
        inputSource = createNewStreamSource(0, estBitrate);
...
::getStreamParameters(unsigned clientSessionId,
  ...
  if (tcpSocketNum < 0) { // UDP
	...
	destinations->addr = destinationAddr;
