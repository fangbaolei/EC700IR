...
//FIXME: delete this
extern void shiftBits(unsigned char* toBasePtr, unsigned toBitOffset,
...
::analyze_vui_parameters(BitVector& bv,
  ...
  if (timing_info_present_flag) {
    ...
    //Here we will enforced set the frame rate as 25.0
    /*
    unsigned num_units_in_tick_value = 1000;
    unsigned time_scale_value = 50000;
    unsigned char *ptmp1 = (unsigned char *)&num_units_in_tick_value;
    unsigned char *ptmp2 = (unsigned char *)&time_scale_value;
    unsigned num_units_in_tick_2000 =
	    (ptmp1[0] << 24) | (ptmp1[1] << 16) | (ptmp1[2] << 8) | ptmp1[3];
    unsigned time_scale_100000 = 
	    (ptmp2[0] << 24) | (ptmp2[1] << 16) | (ptmp2[2] << 8) | ptmp2[3];
	    */

    //XXX set H264 source num_units_in_tick
    //unsigned tmp_num_units_in_tick = bv.getCurBitIndex();
    //shiftBits(fStartOfFrame + fOutputStartCodeSize, tmp_num_units_in_tick,
//	       (unsigned char const* )&num_units_in_tick_2000, 0, 32);

    num_units_in_tick = bv.getBits(32);
    DEBUG_PRINT(num_units_in_tick);

    //XXX set H264 source time_scale
    //unsigned tmp_time_scale = bv.getCurBitIndex();
    //shiftBits(fStartOfFrame + fOutputStartCodeSize, tmp_time_scale,
	//	       (unsigned char const* )&time_scale_100000, 0, 32);
...
#define MAX_NUM_NAL_UNIT	8

unsigned H264or5VideoStreamParser::parse() {
try {
	unsigned NALUnitLength = 0;
	u_int8_t *NALUnitFirstByte = NULL;
	...
	// forms a complete NAL unit, and that there's no 'start code' at the end:
		...
		if (!fHaveSeenFirstByteOfNALUnit) {
			...
			NALUnitFirstByte = getNextToParse();
			fHaveSeenFirstByteOfNALUnit = True;
		}
		int frameSize = getCurFrameSize();
		int lastNALUPos = 0;
		int i = 0;
		...
		saveANALU(i - lastNALUPos);	//the last NALU
		NALUnitLength = i - lastNALUPos;
	}
...
curFrameSize()-fOutputStartCodeSize, nal_unit_type, nal_unit_type_description_h265[nal_unit_type]);
	...
	//TODO: add H265
	nal_unit_type = (*NALUnitFirstByte) & 0x1F;
	...
	if (isVPS(nal_unit_type)) { // Video parameter set
		...
		usingSource()->saveCopyOfVPS(NALUnitFirstByte, NALUnitLength);
		...
		if (fParsedFrameRate == 0.0) {
			...
			// First, save a copy of this NAL unit, in case the downstream object wants to see it:
		usingSource()->saveCopyOfSPS(NALUnitFirstByte, NALUnitLength);
		...
		usingSource()->saveCopyOfPPS(NALUnitFirstByte, NALUnitLength);
...
// (RTP streamers need to know this in order to figure out whether or not to set the "M" bit.)
	...
	*/
	...
	fprintf(stderr, "*****This NAL unit ends the current access unit*****\n");
		...
		//add by wujf
		//修复时间戳问题，出现超过40ms的及时纠正
		u_int64_t nextTick = 0, nowTick = 0;
		nextTick = (nextPT.tv_sec * 1000) + (nextPT.tv_usec / 1000);
		nowTick = (now_tv.tv_sec * 1000) + (now_tv.tv_usec / 1000);
		if((nowTick > nextTick) && ((nowTick - nextTick) > 1000/(usingSource()->fFrameRate))){
			static u_int64_t lastTick = 0;
			static int lastHasBeenIncrement = 0;
			//避免连续几帧过来出现连续多设置时间的问题
			//超过帧间隔的一半才设置
			if((nowTick > lastTick) && (nowTick - lastTick) > (1000 >> 1)/(usingSource()->fFrameRate)){
				if(lastHasBeenIncrement == 0){	//如果连续几帧延迟则隔帧延迟
					double nextFraction = nextPT.tv_usec/1000000.0 + 1/(usingSource()->fFrameRate);
					unsigned nextSecsIncrement = (long)nextFraction;
					nextPT.tv_sec += (long)nextSecsIncrement;
					nextPT.tv_usec = (long)((nextFraction - nextSecsIncrement)*1000000);
					lastHasBeenIncrement = 1;
					fprintf(stderr, "[%s %d]:fix rtp timestamp\n", __FILE__, __LINE__);
				}else{
					lastHasBeenIncrement = 0;
				}
			}
			lastTick = nowTick;
		}
		...
		//避免修改时间小于当前时间
		if(abs(now_tv.tv_sec - nextPT.tv_sec) > 3){
			fprintf(stderr, "[%s %d]:fix rtp timestamp base, time diff:%d\n", __FILE__, __LINE__,
					abs(now_tv.tv_sec - nextPT.tv_sec));
			nextPT.tv_sec = now_tv.tv_sec;
			nextPT.tv_usec = now_tv.tv_usec;
		}
	}
	setParseState();
