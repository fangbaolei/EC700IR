...
public:
	...
	Boolean setUpConnectionStatus(Port connPort);
	...
	void rtcpRRHandler(RTSPClientSession *clientSession);

	void setStreamName(unsigned int index, const char *streamName, unsigned int nameLen);
	void getStreamName(unsigned int index, char *streamName, unsigned int *nameLen);

	void setAutoControlBitrate(int autoControlBitrate){fAutoControlBitrate = autoControlBitrate;};
	int getAutoControlBitrate(){return fAutoControlBitrate;}
...
protected:
	...
	Boolean getConnectionStatusRequestHandler(int clientSocket, ServerMediaSubsession *subsession, E_H264_STATUS eStatusCmd);
...
public: // should be protected, but some old compilers complain otherwise
	...
	protected:
		...
		ServerMediaSession *getServerMediaSession(){ return fOurServerMediaSession;}
...
private:
	...
	static void incomingConnStatusHandler(void*, int);
	void incomingConnStatusHandler1();

	void incomingGetRTSPStatusHandler1(E_H264_STATUS command);
	void incomingConnectionHandler(int serverSocket);

	void reduceBitrateHandler(unsigned int streamIndex);
	void ascendBitrateHandler(unsigned int streamIndex);
...
private:
	...
	int fConnStatusSocket;  //for get connections status 
	int fConnStatusClientSocket;
	int fAutoControlBitrate;
	...
	//for statistics lost packets
	unsigned int fStatisticsInterval;
	unsigned int fNextAscendInterval;
	unsigned int fReduceAdjustThreshold;

	char fStreamName[RTSP_STREAM_MAX_NUM][64];

	struct lostPacketsStatistics{
//		char streamName[64];
		unsigned int lastTotalNumPacketsLost;
		unsigned int lastStatisticsTime;
		unsigned int lastReduceTime;
		unsigned int nextAscendTime;
	} fLostPacketsStatistics[RTSP_STREAM_MAX_NUM];
};
