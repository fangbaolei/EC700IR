...
{ _wsse__UsernameToken *token = soap_wsse_UsernameToken(soap, NULL);
  ...
  { /* password digest or text? */
      ...
      { 
        ...
        */
        ...
        { /* authorize if HA1 and HA2 identical and not replay attack */
        	printf("soap_wsse_verify_Password:compare HA1 to HA2 is OK ,Authentication is success !!! \n");
			return SOAP_OK;
		  #if 0 
		  /*防止重放攻击验证失败，所以暂时屏蔽，只验证密码是否相等*/
	          if (!soap_wsse_session_verify(soap, HA1, token->wsu__Created, token->Nonce))
	          {	
	          	printf("soap_wsse_verify_Password: soap_wsse_session_verify OK  \n");
	            return SOAP_OK;
			  }

	          	printf("soap_wsse_verify_Password: soap_wsse_session_verify eorro  \n");
	          return soap->error; 
		   #endif
        }
      }
    ...
    { /* check password text */
    	printf("soap_wsse_verify_Password:check password text \n");
      if (!strcmp(token->Password->__item, password))
        return SOAP_OK;
    }
  }
	printf("soap_wsse_verify_Password  wsse__FailedAuthentication \n");
